...
window.WIKI_SEARCH_MANIFEST = "3b21fb25b8b1f537";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "发布与构建 除非用户明确要求，否则不要创建 release。 用户约定（2026-09-24）：今后用户说“打包”，默认要求完成构建并发布正式 GitHub Release，构成发布授权；不得只触发 workflow_dispatch 产物构建就视为完成。明确说“只构建”“不发布”时按限定范围执行。仍须遵循下述 main 分支、release.sh、版本规则、预检、annotated tag 与发布验证流程。该约定面向后续请求，不自动把此前已触发的仅构建任务升级为发布。 构建发布流程 main 候选提交先由官方 Flutter SDK 完成跨平台预检，成功后 release.sh 才更新 pubspec 并创建 annotated tag，GitHub Actions 再签名和发布 Android APK 与 macOS arm64 产物。 pubspec.yaml X.Y.Z+N 唯一版本来源 scripts/release.sh 仅 main · 发布前预检 annotated tag vX.Y.Z 发布足迹 releases.html 追加 GitHub Actions tag 触发 · Node 24 Actions Android APK 签名对齐 · versionCode macOS arm64 macos-26 runner · SwiftPM GitHub Release release notes 校验 当前发布流程： 使用 scripts/release.sh 。用户约定：确认完整发布流程已启动后返回链接，不持续等待构建结束；预检仍需执行，可让脚本在后台等待并于预检通过后继续创建 tag、触发正式发布。不要把启动状态记录为发布成功。 脚本只允许在 main 分支运行；不要从功能分支直接创建发布 tag。 使用 --push 时，脚本先把当前干净的 main 候选提交推送到远端，并等待 Release Preflight 使用固定的官方 Flutter SDK 完成静态分析、完整测试、 Android Debug 和 macOS Release 冒烟构建。只有预检成功后，才修改版本号、 写入发布足迹和创建 tag。 预检失败时不会产生版本提交或正式 tag。修复后重新运行同一条发布命令即可， 不要绕过预检手工打 tag。 release tag 必须是 annotated tag。 GitHub Actions 会从 tag 构建 Android APK 和 macOS arm64 产物。 本地和 GitHub Actions 统一使用 Flutter 3.47.0 ；项目最低约束为 Flutter >=3.47.0 、Dart ^3.13.0 。升级 SDK 时必须同步检查 pubspec.yaml 、lockfile 和两个工作流中的 Flutter pin，不能只改本机。 GitHub 预检以干净的官方 SDK 为基线，再执行仓库跟踪的 scripts/apply_flutter_patches.sh ；这是发布兼容性与视觉一致性的权威结果。 当前补丁只包含文章选择统一行高、 WidgetSpan.rawText 及其复制映射，不得把 本机 Flutter SDK 中其他未审计实验修改混入发布。补丁固定校验 framework revision，不匹配时必须先审计迁移，不能模糊应用。 macOS 发布包必须保持 arm64。 macOS job 固定使用 macos-26 ARM64 runner。原生侧边栏使用了 macOS 26 SDK 的 NSGlassEffectView ，不要改回默认仍可能选择 Xcode 16 的 macos-latest ，否则 CI 可能在 Swift 编译阶段失败。 macOS 原生插件已迁移为纯 Swift Package Manager。仓库不再保留 macos/Podfile 、 Podfile.lock 或 Xcode Pods 引用； screen_retriever 至少保持 0.2.2 ，该版本修复了其 macOS SwiftPM manifest。若构建再次输出 Running pod install ，应先排查是否误恢复 CocoaPods 集成。 每次发布的可复现命令由 scripts/release.sh 追加到 docs/agent_handoff/history/releases.html （内嵌 Markdown）；根 AGENT_HANDOFF.md 始终保持为短入口。 依赖维护（2026-09-25） pub get 的 available / newer versions incompatible with dependency constraints 列表是版本提醒，不能等同于解析失败；必须看是否 Got dependencies，以及实际 error。用户原粘贴日志真正阻断运行的是 Xcode 许可未接受，已另行解决。锁文件用于固定可复现依赖，不为了清除提示一键升级所有包。 本次优先提高 dio 最低版本至 ^5.11.1、 flutter_cache_manager 至 ^3.4.5；解析只更新这两项及后者要求的 uuid 4.6.0。缓存管理器修复图片缩放解码错误传递、缓存元数据写入与删除路径，Dio 修复流式背压和请求/拦截器挂起；不能将这些上游修复直接认定为此前卡图的根因。继续保留应用自有图片下载超时、缓存目录与动图暂停逻辑。 其余依赖安排：路径/窗口等小版本需按对应原生变更单独验证；cached_network_image 4、分享、权限、取色及弹窗大版本需要迁移评估；video_player 和 WebView 虽有约束内更新，也需媒体播放/登录回归后再升级。SDK 固定及工程 Flutter 选择补丁保持原样，SDK 固定的传递包不使用 dependency_overrides 强行追新。升级后仍有 76 项新版提示是预期，Flutter SDK 升级提示也不自动构成升级任务。 本机 Xcode 组件初始化（如 sudo xcodebuild -runFirstLaunch）只影响本机，不会经 Git 同步到 GitHub Actions；pubspec.yaml 与 pubspec.lock 的依赖变更提交推送后才影响后续云端构建。本轮仅提交推送，不触发构建工作流、不修改版本号或创建 tag。 版本号规则 pubspec.yaml 的 version: X.Y.Z+N 是唯一版本来源： X.Y.Z 是用户可见版本，并对应 annotated tag vX.Y.Z 。 N 是单调递增的内部构建号；Android 使用它作为 versionCode ，macOS 使用它作为 CFBundleVersion 。 Major、Minor 或 Patch 变化时都不能重置构建号。 普通 commit 不修改版本号；只有真正创建发布时才由 scripts/release.sh 更新版本并把构建号加一。 不要手工同步 Android、macOS、设置页或请求头中的版本；这些位置应继续从 "
 },
 {
