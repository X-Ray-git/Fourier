...
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "后台任务 任务中心在 macOS 和 Android 上都可见，用于展示正在进行或排队中的工作。 当前设计 应和设置页共享同一种轻量面板语言。 列表内部避免密集 Liquid Glass 表面；使用简单描边和微弱 hover 状态。 作为 overlay 打开时，背景应保证可读。 Scrollbar 不应造成左右 padding 不对称或出现重复条。 Android 保持设置页进入的二级页面，使用 MobileBlurAppBar 、 12px 页面边距和 MobileSettingsPanel 轻量外壳。AI 失败记录继续作为下一级页面；不要用旧式透明 Card 或密集实时玻璃重做。 交互注意点 macOS 如需保留上下文可以继续使用 overlay；Android 已确认采用完整二级页面，不应为了跨平台形式一致强行改成 overlay。 “去审核”等操作应符合当前 macOS 控件语言，不要使用旧的移动端按钮样式。 如果某个操作被刻意隐藏或移除，应记录产品原因；否则把它当作 bug。 AI 队列：滚动补位调度 三个 AI worker（自动翻译 / 自动摘要 / AI 过滤）与全文抓取队列采用 滚动补位 调度： 任一任务完成后 立即 补充一个新任务，运行中数量始终不超过各自并发配置；不再整批取并发数后 Future.wait ，慢任务不再阻塞后续任务。 三个 worker 保持完全独立：独立的队列、独立的并发配置（ LlmConfig 的 translate / summary / filter 前缀）与独立生命周期，不合并成共享调度器。 并发数在每次补位时按最新配置读取；运行中修改并发数在后续补位时生效，不中断正在运行的任务。 取消（账号切换时由 AccountDataService 调用）清空队列与运行中集合，并推进各 Worker 自己的运行代次；旧代次任务可以自然结束，但其完成回调不得删除新代次的同 entryId 运行标记或触发补位。账号 revision 失效后的在途结果不落库，全文抓取的旧失败结果也不得写失败状态或安排重试。 AI 过滤出队时按最新持久化状态跳过已读 / 已判定 / 已捞回文章。 全文抓取（ AutoReadabilityWorker ）为并发 3 的滚动补位队列。 全文抓取成功标记与重试 成功标记 readability_fetched_<entryId> 只在成功解析并持久化有效正文后写入 ；失败不再写标记，因此可重试。 失败登记可诊断状态 readability_fetch_state_<entryId> （attempts / lastError / lastAt），自动重试有限次数 + 指数退避（2s / 5s），达到上限后停止并保留失败状态。 旧版本在请求前就打标记的历史数据 不批量清除 ；只有正文为空或明确失败且文章仍未读时进入有限次兼容重试。 已开始抓取的文章即使完成前被标为已读，也允许这一条已开始的流水线继续进入翻译和摘要（ AutoAiQueueCoordinator.onArticleContentAvailable(allowRead: true) ）；尚未开始的等待任务在出队时若已标为已读则移除。 打开文章只记录最近阅读，不得因此标记已读。 关系建立队列 文章关系是摘要完成后的独立增益链路，不属于现有 AI 质量过滤，也不与翻译、摘要、过滤三个滚动补位 worker 合并： 第一次启用时写入账号级 activatedAt ；只消费此后完成并持久化的结构化摘要，启用前旧摘要不会批量进入关系判断。摘要正文缺失、失败或仍 pending 时不入队。 每次最多取 32 篇新摘要；历史窗口从 0 开始随成功批次增长。请求输入是“本批新摘要 + 当前历史窗口”的一个整体，不生成 32×256 个请求。 worker 固定单飞。凑满 32 篇立即发车；摘要 worker 完全空闲后，不足 32 篇的尾批也立即发车。冷启动本身不得反复冲刷尾批；失败、空响应、非法 JSON 或输出截断时保留 pending，且不推进历史窗口，重启后恢复任务中心错误提示并等待用户明确重试。 手动生成或重试摘要不依赖自动摘要 worker 的生命周期；单篇完成后会显式请求处理当前尾批，避免少于 32 篇时必须等到重启。 DeepSeek 使用 JSON Object 模式。批次记录保存模型、Prompt/schema 版本、耗时、组数以及 prompt/completion/cache hit/cache miss token，供后续真实数据分析。 关系 Prompt 已纳入设置与 version 1 配置导入导出；每次请求的 Prompt 版本使用内容指纹区分，修改 Prompt 后不会把新旧关系结果误当成同一实验口径。 第一版算法参数仍固定为每批最多 32 篇新摘要、历史阈值 256、单批串行。设置页将这些禁用字段并入“关系判断 LLM 参数”，避免把同一条关系流水线拆成两个配置模块；真实关系质量、token 量级和请求成本充分验证前不开放修改。 请求序列化时把稳定的 history 数组放在变化的 new 数组之前，以尽量利用 DeepSeek 自动前缀缓存。成功追加后仅当历史数超过 256 才从队首固定淘汰 32 篇，因此窗口会在约 225～256 篇间波动：例如 256 + 1 - 32 = 225 ，随后再新增 20 篇为 245。分块淘汰让小批量更新之间保留更长的稳定前缀；发生淘汰的那一次仍会因 history 起点变化而降低缓存命中，不能把该策略描述为始终命中缓存。 pending、历史窗口、关系节点/组和批次使用账号级 Hive 数据；账号退出或切换时清理，不进入设置导入导出。 任务中心“关系建立”展示排队数、当前 new + history 、本次完成批次、历史窗口、关系组和最近错误，并允许安全重试。 当前阶段只记录、展示和统计关系， 不会 自动改变已读、人工动作或垃圾拦截状态。未来自动连带拦截仍是独立待验证决策。 DeepSeek 请求用量账本 LlmUsageLedger 为翻译、摘要、质量过滤和关系建立提供统一的请求级 Hive 账本（box llmUsageEvents ）： 每一次真实 HTTP 尝试单独记录，重试和分块翻译不会被合并；响应成功但本地解析失败时仍保留已经产生的 token 用量。 记录任务类型、文章/批次/分块标识、请求次数、耗时、结果、请求与实际模型、thinking/reasoning 参数、HTTP 状态、provider request id、finish reason、system fingerprint，以及 prompt/cache hit/cache miss/completion/reasoning/total token。 只保存 Prompt 的 SHA-256 指纹，不保存 Prompt、正文、摘要、译文、模型输出或 reasoning 内容；错误文本截断为 500 字符。 LlmUsageLedger.summarize() 可按全部任务或单一任务汇总请求数、成功/失败数、各类 token 和缓"
 },
 {
 ...
 },
 {
  ...
  "title": "测试",
  "headings": [],
  "text": "测试 主要命令： dart analyze lib test flutter analyze lib test flutter test flutter build macos --debug 预期注意点： Flutter 命令可能需要权限更新 Flutter SDK cache。 完整 dart analyze 会很吵，因为 reference/ 包含外部复制来的参考工程。 Flutter 3.44.6 的分析器会对现有 renderer 和少量 helper 报 prefer_initializing_formals info；工具链迁移时共观察到 59 条，没有 warning/error。CI 使用 flutter analyze --no-fatal-infos lib test 。不要为了消除本次升级带来的 info 顺带机械重写液态玻璃 renderer；后续可作为独立无行为重构处理。 当前测试： test/article_content_utils_test.dart test/html_chunk_parser_test.dart test/article_card_test.dart test/article_model_test.dart test/feed_model_test.dart test/html_entity_utils_test.dart test/implicitly_animated_list_test.dart test/widget_test.dart 正文来源兼容回归至少应覆盖：目标来源作者区被转换、作者姓名/账号/主页和头像地址保留、装饰头像空壳被清理、其他来源的圆形/头像式图片不被误删，以及内部作者标记只生成一个 authorList chunk。 修改单个功能时，先运行相关窄测试；推送前再运行完整项目测试套件。 macOS 列表动画诊断埋点默认关闭。普通 Debug/Release 不打印，也不会挂载动画监听器。复现 M/K 、审核横滑或主时间线双击偶发瞬移时，运行： flutter run -d macos --no-pub \\ --dart-define=AUTO_FOLO_ANIMATION_PROBE=true \\ 2>&1 | tee /tmp/fourier-animation.log 按页面过滤： grep ReviewAnimProbe /tmp/fourier-animation.log grep TimelineAnimationProbe /tmp/fourier-animation.log grep TimelineTapProbe /tmp/fourier-animation.log grep TimelineListResetProbe /tmp/fourier-animation.log grep TimelineReadStateProbe /tmp/fourier-animation.log TimelineAnimationProbe 同时覆盖主时间线的 M 、双击、退场动画进度和滚动 offset； TimelineTapProbe 记录原始点击与双击识别，用于区分手势未识别和动画未绘制； TimelineListResetProbe 记录零时长批量同步的明确原因，用于确认单篇读状态是否被错误归入批量路径； TimelineReadStateProbe 区分已读状态被未读快照降级与列表组件的纯视觉残影。正常单篇移除应依次出现动画 bucket 4→3→2→1→0 ，增量可见列表更新不应触发 list reset，外部浏览器只能在 remove.end 后打开。埋点只输出文章 id 的末 8 位、动作来源、列表数量、动画阶段和慢帧耗时，不输出标题、正文、凭据或完整文章 id。不要为了普通运行长期打开该开关；同步 debugPrintSynchronously 本身可能轻微干扰 Debug 性能测量。"
 },
 {
