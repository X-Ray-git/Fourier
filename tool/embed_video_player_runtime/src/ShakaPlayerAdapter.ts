...
export class ShakaPlayerAdapter implements SabrPlayerAdapter {
  ...
  private parseRequest(
    ...
    asMap<string, string>(request.headers).forEach((value, key) => {
  ...
  private async handleUmpResponse(
    ...
    if (!response.body) {
      ...
      while (!abortController.signal.aborted) {
        ...
        if (done) {
          // If we got here, we read the whole response but there was no
          // segment data; it means we must follow a
  ...
  private async request(
    ...
    try {
      ...
      if (isGoogleVideoURL(uri)) {
        fetchUri = buildProxyURL(uri);
      }
      const response = await fetch(fetchUri, init);
  ...
  public disableSabrInterceptors(): void {
  ...
  public createShakaResponse(args: ShakaResponseArgs): shaka.extern.Response {
    return makeResponse(
      ...
      args.uri,
      isGoogleVideoURL(args.uri) ? args.uri : args.response.url,
      args.request,
      args.requestType
    );
  }
