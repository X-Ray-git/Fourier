...
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "title": "路由与状态",
  "headings": [],
  "text": "路由与状态 路由： 路由定义位于 lib/router/app_pages.dart 。 主路由通过 MainController 承载类似 tab 的各个区域。 MainController.currentIndex 的 0/1/3 在两端分别都是时间线、垃圾拦截和设置，但索引 2 有平台差异：Android 是订阅源，macOS 是最近阅读。不要在未检查平台的共享代码中把 2 硬编码为同一页面。 Android 主 shell 的四个常驻页面依次为时间线、垃圾拦截、订阅源和设置；垃圾拦截用 embeddedInMainNavigation 关闭自己的独立 AppBar。独立 Routes.filterReview 仍保留兼容入口。 状态： GetX 响应式值（ Rx 、 Obx ）是主要模式。 Service 经常暴露静态方法，并在内部维护响应式 map。 ArticleStateNotifier 用于跨页面刷新文章状态。 macOS 相关文章详情栈： TimelinePage 、订阅源详情、垃圾拦截和最近阅读等 macOS 分栏入口都可以通过 MacArticleDetailStack 在右侧详情栏内打开相关文章； _ArticleRelationsSection 在 macOS 走这个回调，Android 仍走独立的 Get.toNamed(Routes.article) 全屏路由。 MacArticleDetailStack 使用内嵌 Navigator 。打开相关文章时保留上一层 ArticlePageView 挂载在路由栈中，因此上一层的滚动位置、目录状态和已解析内容可以在 Esc 返回后继续使用；不要为了修复视觉问题把它改成外层选中项替换或统一成 Android 的全屏路由。 macOS 相关文章路由在 lib/pages/article/article_page.dart 的 _MacArticleDetailStackState._openRelatedArticle() 中创建。目标页仍会独立创建 ArticleController ，其正文会经过 ArticleController._initContent() 和 Isolate.run() 异步规范化/分块；异步回调受 controller 代次和账号 revision 守卫，不能在页面失效后回写 UI。 当前相关文章路由恢复功能最初的 PageRouteBuilder 视觉：push 为 160ms 、约 2.5% 水平滑入并透明淡入，pop 为 140ms 反向滑出/淡出；正向使用 easeOutCubic ，反向使用 easeInCubic 。2026-09-03 曾加入的不透明 surface 前景解决了当时推断的异步交叉闪烁，但用户确认其实体表面滑入观感不如初版；2026-09-12 将其移除并进入真机观察阶段。 Esc 直接弹出路由并进入上述反向过渡；相关文章页面的 M 或工具栏标为已读动作通过 _popRelatedArticleAfterFrame() 在当前帧结束后弹出，先让本地读状态、Hive/read-sync 和跨页面通知完成一帧渲染。 这条延迟只用于“标为已读后返回”的相关文章回调，不改变普通文章的下一篇策略、恢复未读语义、Android 应用级返回或主时间线列表的 180ms 行级退场动画。完整演进见 2026-09-02 阶段性决策 、 2026-09-03 不透明转场决策 与 2026-09-12 的初版视觉恢复记录。 重要体验规则： 当 service 记录被删除时，避免继续回退到过期 controller 缓存。近期 bug：从卡片右键菜单删除摘要后， SummaryService 记录已删除，但文章详情仍显示缓存的 controller.summaryText 。修复后的行为是：可见摘要状态应信任 service 记录。"
 },
 {
 ...
 },
 {
  ...
  "headings": [
   {
    "id": "图片慢加载与超时2026-09-24",
    "text": "图片慢加载与超时（2026-09-24）"
   },
   {
    "id": "选择结果跨异步刷新保持稳定",
    "text": "选择结果跨异步刷新保持稳定"
 ...
 },
 {
  ...
  "headings": [
   {
    "id": "2026-09-26相关文章返回与审核选中生命周期",
    "text": "2026-09-26：相关文章返回与审核选中生命周期"
   },
   {
    "id": "2026-09-25-正文区域显示拒绝理由",
    "text": "2026-09-25 正文区域显示拒绝理由"
   },
   {
    "id": "文案语义与事件账本",
    "text": "文案语义与事件账本"
  ...
  "text": "2026-09-26：相关文章返回与审核选中生命周期 用户报告：审核页进入相关文章后，偶发 M 返回和左侧选中异常，可能与目标是否在审核列表有关。代码确认二者有正常差异：从左侧 A 打开 B，左侧 A 是返回锚点；B 使用普通阅读上下文，M 将未读 B 标已读并返回上一层，不调用 A 的审核移除；B 在审核列表内时会随已读同步退出列表，列表外则不改变审核成员。K/N 不穿透作用于 A。不改这套交互，也不凭这些正常差异断言复现了用户那一次故障。 排查修复： 文章控制器仍按 entryId 共享，以兼容撤销、菜单和时间线的现有查找；但由所有挂载视图引用计数管理。Get 注册为 permanent 防止单条路由自动删除，最后一个视图释放时才 force delete。A→B→A 关闭一层不会销毁根 A 正在使用的控制器；同 ID 已换成新实例时，迟到释放不删除新实例。图片活跃标记同样等到最后一个视图释放。 导航状态改由 ArticleDetailNavigation 观察当前栈的 push/pop/remove/replace，取消 popped Future 的深度计数回调；dispose 不向其他栈发送 false，新栈主动初始化状态。审核页只接收属于当前左侧选中项的状态回调；审核和普通时间线的详情 isActive 同时校验所属选中项，防止过渡中的旧页面响应按键或菜单。 每次关闭/标已读返回绑定具体 Route，只有该 Route 仍是当前顶层时才可 pop。帧后返回去重并主动 scheduleFrame，解决界面静止时仅 addPostFrameCallback 无帧可等的问题；若期间打开新文章、已返回或栈已销毁，旧回调失效，不误关新的顶层。 列表移除动画完成时，只有当前仍选中被移除文章才自动选后继；用户期间手动选另一篇或 Esc 清空时不抢回选中。普通审核后继选择及 Undo 恢复行为保留。 受控测试覆盖多层返回、重复返回、静止帧后返回、插入新路由/销毁旧栈、同文多视图释放、动画中切换/清空选中，以及移除列表内/外的非选中文章不改变锚点。没有对真实文章执行 M/K/N；真实快捷键、动画与现场偶发现象仍待新构建使用验证。 2026-09-25 正文区域显示拒绝理由 用户要求左侧审核卡片保持原样，并在右侧正文区域像摘要一样展示完整拒绝理由。复用现有 ArticlePageView 的 isReviewContext，仅审核上下文且 filterReason 非空时在相关文章之后、文章摘要之前显示理由卡；独立于摘要显隐开关，不按文章存在历史理由就在其他入口显示。从审核页进入相关文章继续使用普通阅读上下文，不继承原文理由。 摘要和理由共用 widgets/article_info_card.dart 的 ArticleInfoCard，统一标题、图标、间距、浅深色背景与 StableSelectableHtml 文本选择/复制行为，正文文本经 normalizePlainText 处理。组件只接收展示数据，不读取 Hive、不控制审核或摘要状态；调用方负责数据和可见性。拒绝理由使用橙色，摘要保留既有样式；没有另建审核正文页，也没有引入通用页面配置框架。此次不改变 Android 审核入口或左侧卡片。 垃圾拦截与审核 垃圾拦截审核流转 文章经 AI 判定进入审核页，K/M/N 三种动作写入 userAction 统计信号；误分类是保留或拒绝与已读的原子组合。 文章流 时间线 / 订阅源 AI 过滤判定 DeepSeek · 可编辑 Prompt 被拒绝？ 保留在 普通时间线 垃圾拦截审核页 独立审核队列 · 横滑 / 快捷键 / 右键 K 保留 写 userAction='k' 保留 filterReason M 确认拒绝 写 userAction='m' 弱信号 N 误分类（旗帜） 拦截页 n_keep / 时间线 n_spam 原子撤销：分类 + 已读 垃圾拦截/审核页用于审核被 AI 拒绝的文章。 关键预期： 用户可以审核被拒绝文章。 Android 将垃圾拦截作为四项底部主导航中的一级入口；常驻实例使用嵌入模式，不创建第二层 AppBar。独立路由继续可用，但时间线顶部不再保留重复的“AI 智能过滤”入口卡片。 Android 时间线角标与垃圾拦截角标允许重叠：被 AI 拒绝的未读文章仍按既有逻辑留在普通时间线，同时进入待审核列表。时间线数字使用非静默未读总数，垃圾拦截数字使用 isRejectedByAi && !isRead ；这不是互斥队列。 移除/通过条目后，应跳转到下一篇合适文章，并在 macOS 上保持焦点行为可用。 Header 和面板样式应遵循当前 macOS 轻量面板语言。 Header 不再使用盾牌图标；macOS 与 Android 都分别显示“质量过滤”和“关系建立”状态。质量过滤显示待处理数量或完成态；关系队列收集时显示待批数量/128，API 批次运行时显示独立转圈状态，不能再用一个加载图标混合表达两条流水线。 文章卡片/审核行应保持和时间线卡片一致的交互反馈。 垃圾拦截页不应成为一套完全分叉的时间线实现；能和普通时间线共享的时间线级操作、按钮和文章动作菜单应尽量共享。 质量过滤的多模态边界： 首轮仍按标题、来源和正文前 500 字执行用户配置的质量规则；纯文本模型只有在图片很可能提供决定性证据时才返回 needs_visual_context=true ，随后由质量过滤任务当前选择的视觉模型复核。 图片提取、顺序、去重、SVG 排除和每次最多 8 张的上限与摘要完全共用 ArticleVisualContextService ，但过滤和摘要仍是独立 Worker，不能因为图片规则相同而合并调度。 过滤业务 Prompt 可编辑；程序响应协议只读、随版本维护且不导出。视觉模型从受支持白名单中选择并进入设置备份；旧备份缺少该项时回落默认模型，旧 Prompt 缺少转交字段而文章有图片时，程序执行保守视觉复核。 视觉请求最终失败必须保留文章，并使用明确的“视觉信息暂不可用”理由；网络或图片可用性不能成为自动拒绝依据。 状态字段： ArticleModel.isRejectedByAi ArticleModel.filterReason ArticleModel.filterReviewed ArticleModel.filteredAt ArticleModel.userAction ：用户动作标记，事后统计误分类用。取值 'k' （拦截页保留）、 'm' （拦截页确认拒绝）、 'n_keep' （拦截页误分类：保留+已读）、 'n_spam' （常规页误分类：拒绝+已读）、null（未表态）。同一文章多次动作时 latest wins。 统计仅覆盖当前 articleDb 中仍保留的近期文章，不是永久操作历史。它受 5000 篇缓存上限和账号数据清理约束；这是用户确认的产品边界，不要为此另建长期日志或把记录加入设置备份。 本地数据库 merge 时不要丢失这些字段。 误分类（ N / 右上角旗帜按钮）： 拦截页语义：当前文章应该保留，但用户已读完 → 保留 +"
 },
 {
 ...
 },
 {
  ...
  "headings": [
   ...
   {
    "id": "微博历史视频排查记录2026-09-03",
    "text": "微博历史视频排查记录（2026-09-03）"
   },
   {
    "id": "文章图片缓存统一成功通知",
    "text": "文章图片缓存统一成功通知"
 ...
 },
 {
  ...
  "headings": [
   ...
   {
    "id": "2026-09-03-图片下载失败修复边界",
    "text": "2026-09-03 图片下载失败修复边界"
   },
   {
    "id": "摘要完成后的关系建立",
    "text": "摘要完成后的关系建立"
  ...
  "text": "翻译与摘要 自动 AI 队列调度 正文持久化后经 onArticleContentAvailable 统一入队：只服务当时仍未读的文章，按订阅源开关决定翻译，Worker 并发执行并写入 Hive；手动操作不受限制。 正文已持久化 Readability / 详情页补抓 onArticleContentAvailable 唯一自动入队门卫 当时仍未读？ 已读则不排队 摘要队列 全部未读文章 翻译队列 按订阅源开关 入队后 不追已读 手动翻译/摘要 不受队列限制 AutoAiWorker 并发执行 · 原地重试 Hive 记录 translations / summaries 页面 立即可见 自动任务只在入队时检查文章的最新已读状态。已经是已读的文章不进入自动翻译和摘要队列；以未读状态入队后，后续即使标记为已读也不追踪取消，任务照常完成。这是经长期使用后确认的取舍：旧取消机制只能命中尚未被高并发 Worker 取入批次的少量任务，收益不足以支撑额外状态分支。手动翻译/摘要和垃圾拦截判定均不受影响。 新正文可用后的自动 AI 流转由 AutoAiQueueCoordinator.onArticleContentAvailable() 统一负责。后台 Readability、文章详情页直接补抓全文和 Inbox 详情补正文都必须在成功持久化正文后调用该入口；入口读取数据库中的最新已读状态，只给当时仍未读的文章排摘要，并按订阅源自动翻译开关排翻译。不要让详情页只更新显示和数据库而遗漏 AI 队列，也不要在 setReadState() 中重新加入队列取消副作用。 刷新后入队的 ArticleModel 可能仍是上游 RSS 短摘要，即使 LocalArticleDbService.upsertMany() 已在数据库中保住先前抓取的全文。 AutoReadabilityWorker 开始处理时必须调用 preferPersistedContent() ，让过滤、摘要和翻译看到同一份更完整正文。否则一次长文分块翻译失败后，后续刷新可能用一句话短摘要生成合法的完成态译文，并永久阻止自动重译。 确定性 Provider 拒绝 LlmFailurePolicy 集中解析 DeepSeek 的结构化错误。当前已确认 HTTP 400 / Content Exists Risk 属于相同请求不经修改便无法恢复的内容安全拒绝，不应当与超时、断网或服务端临时故障使用同一重试策略。 摘要、翻译（含长文章分块翻译）和质量过滤遇到该错误时只请求一次；摘要与翻译保存错误态并显示“DeepSeek 内容安全检查拒绝处理此文章”，不得通过删词、改写正文或隐藏来源来绕过 Provider 检查。 LlmAutoRetryBlockService 按“任务类型 + 文章 ID”将阻断状态存入账号级 localCache 。三个自动 Worker 在入队前统一检查，因此后续启动或刷新不会反复消耗请求；切换账号时随派生数据清除，不进入设置配置备份。 用户从正文、右键菜单或任务中心明确触发摘要/翻译时视为手动重试，先解除对应阻断，再按当前正文和配置发起新请求。普通网络错误仍沿用原有有限重试。 如果纯文本摘要已经成功、只有可选的视觉复核被内容安全检查拒绝，则保留纯文本摘要，不把整篇文章标为失败；质量过滤的视觉阶段同样继续保守保留文章。 翻译 JSON 的有限恢复 2026-09-12 的真实用量账本与受控请求确认：DeepSeek V4.1 Flash 即使收到 response_format: json_object ，仍可能在 translated_html 字符串内输出未经 JSON 转义的真实换行，或把百分号写成非法的 \\% 转义。接口会返回 HTTP 200、 finish_reason: stop 和完整可读译文，但严格 jsonDecode 会失败；少数文章因此被自动 Worker 反复重试。 翻译服务仍以严格 JSON 解析为首选。只有严格解析失败时，才对 JSON 字符串内部 的裸控制字符和非标准反斜杠转义做一次字符级修复，然后重新执行完整 JSON 解析。修复不能补齐缺失引号、大括号或截断内容，最终结果仍必须是对象且包含非空 translated_html 。 普通翻译与长文分块翻译共用同一解析入口。回归测试必须同时覆盖 HTML 属性中的合法 \\\" 、裸换行、非法 \\% ，并确认真正截断的响应仍进入错误态。不要把该恢复扩展成任意 HTML 猜测或宽松字符串拼接，否则会掩盖真实截断与 Provider 协议变化。 相关服务： lib/services/translation_service.dart lib/services/summary_service.dart lib/services/llm_config.dart lib/services/article_relation_service.dart lib/services/article_relation_worker.dart 存储： 翻译记录在 GStorage.translations 。 摘要记录在 GStorage.summaries 。 摘要启用后产生的关系节点/组在 GStorage.articleRelations ，批次与 token/cache 统计在 GStorage.relationBatches 。 LLM 默认取向： 翻译： deepseek-flash 、不思考、默认并发 32、低 temperature、大输出。 摘要： deepseek-flash 、thinking max、默认并发 32、紧凑输出。 过滤： deepseek-flash 、thinking max、默认并发 32；仓库默认只做保守、通用的内容质量审核，不表达特定用户的主题或来源偏好。用户保存/导入的自定义过滤 Prompt 优先于默认值。 关系判断： deepseek-flash 、thinking max、每批最多 128 篇新摘要，网络请求固定单飞。这里的 128 是批大小而不是并发数；历史阈值为 2048，超限时从队首固定淘汰 1024 篇。 摘要与质量过滤的多模态转交 摘要和质量过滤都先调用各自现有的纯文本模型。纯文本阶段通过程序内置字段 needs_visual_context 判断：只有正文文字不足以可靠完成任务、且正文图片很可能承载缺失的实质信息时，才转交各自配置的视觉模型。普通配图不会仅因存在而触发第二次请求。 DeepSeek V4.1 Flash 的规范 API 模型 ID 为 deepseek-flash ，同时支持文本与视觉输入。摘要与质量过滤仍分别保存视觉模型选择，设置页沿用既有模型选择控件；当前视觉白名单只有 deepseek-flash ，后续扩充白名单即可直接增加选项。该选择进入设置导入导出，旧备份缺少配置时回落默认模型，不受支持的导入值会被拒绝。 原生多模态模型不等于所有请求都应传图。现有“纯文本先行、按协议转交视觉”的两阶段策略继续保留，避免普通文章承担图片下载、Base64 编码、请求体积和视觉推理开销；文本阶段"
 },
 {
 ...
 },
 {
  ...
  "headings": [
   {
    "id": "2026-09-26修复审核页相关文章导航边界不改变-m-语义",
    "text": "2026-09-26：修复审核页相关文章导航边界，不改变 M 语义"
   },
   {
    "id": "2026-09-26相关文章采用共享信息框",
    "text": "2026-09-26：相关文章采用共享信息框"
   },
   {
    "id": "2026-09-26关系收缩为近似重复历史全部保留并统一类型",
    "text": "2026-09-26：关系收缩为近似重复，历史全部保留并统一类型"
   },
   {
    "id": "2026-09-24图片挂起请求设定边界并明确等待状态",
    "text": "2026-09-24：图片挂起请求设定边界并明确等待状态"
   },
   {
    "id": "2026-09-24单个静默来源不隐含未分组筛选",
    "text": "2026-09-24：单个静默来源不隐含未分组筛选"
   },
   {
    "id": "用一句话主题锚定事件组取消重叠合并2026-09-24",
    "text": "用一句话主题锚定事件组，取消重叠合并（2026-09-24）"
   },
   {
    "id": "修复代理动图识别复用已有播放门控2026-09-19",
    "text": "修复代理动图识别，复用已有播放门控（2026-09-19）"
   },
   {
    "id": "appinn-摘要指令的完整节点组清理2026-09-18",
    "text": "Appinn 摘要指令的完整节点组清理（2026-09-18）"
   },
   {
    "id": "paul-graham-旧式文章空正文的来源限定回退2026-09-17",
    "text": "Paul Graham 旧式文章空正文的来源限定回退（2026-09-17）"
   },
   {
    "id": "用-widgetspanrawtext-恢复行内代码视觉并限制图片占位动画2026-09-12",
    "text": "用 WidgetSpan.rawText 恢复行内代码视觉，并限制图片占位动画（2026-09-12）"
   },
   {
    "id": "关系组弹窗关闭后再打开相关文章2026-09-12",
    "text": "关系组弹窗关闭后再打开相关文章（2026-09-12）"
   },
   {
    "id": "恢复-macos-相关文章最初的透明转场作为第一阶段验证2026-09-12",
    "text": "恢复 macOS 相关文章最初的透明转场作为第一阶段验证（2026-09-12）"
   },
   {
    "id": "本地静默分组与-android-批量功能边界2026-09-04",
    "text": "本地静默分组与 Android 批量功能边界（2026-09-04）"
   },
   {
    "id": "应用整体迁移为-fourier",
   ...
   {
    "id": "普通视频区分签名过期与通用错误2026-09-03",
    "text": "普通视频区分签名过期与通用错误（2026-09-03）"
   },
   {
    "id": "最近阅读点击不立即重排",
    "text": "最近阅读点击不立即重排"
   ...
   {
    "id": "关系历史窗口与淘汰步长解耦2026-09-08",
    "text": "关系历史窗口与淘汰步长解耦（2026-09-08）"
   },
   {
   ...
   {
    "id": "deepseek-v41-flash-使用规范模型-id并保留按需视觉转交2026-09-11",
    "text": "DeepSeek V4.1 Flash 使用规范模型 ID，并保留按需视觉转交（2026-09-11）"
   }
  ],
  "text": "2026-09-26：修复审核页相关文章导航边界，不改变 M 语义 用户描述偶发 M 返回与左侧选中异常，无法提供精确复现。整体排查确认控制器按文章 ID 共享却按单视图删除、旧栈异步深度回调、无帧请求的帧后返回、动画结束无条件选择后继四类缺口。采用视图引用计数保留现有控制器查找协议，NavigatorObserver 管理栈状态，Route 身份约束延迟返回，列表后继选择尊重用户最新选择；同时限制旧详情的按键/菜单活跃条件。没有扩展为新导航模型或自动把相关文章选中同步到左侧。审核中的关联页仍为普通阅读，M 操作当前关联文章；左侧根文章仍是返回锚点。列表内目标已读后离开审核队列是正常变化，不是关系类型或新配色导致的问题。 2026-09-26：相关文章采用共享信息框 用户确认以“相关文章”为标题，外观与“文章摘要”“拒绝理由”统一：一句话概述作为框内普通正文，下面是有内边距的文章横条，末尾保留展开/收起。为避免嵌套框和复制外观代码，给现有 ArticleInfoCard 增加结构化 child 构造入口；原 text 构造继续服务摘要和理由，保留稳定 HTML 选择/链接渲染。关系模块只负责组概述、文章行、折叠和导航，外框统一维护标题、背景、圆角和间距。没有关联时不显示空框，旧空概述占位及历史多组概述保留。本次替代前面“独立概述横幅”的展示方案，不改变关系判定或数据。 2026-09-26：关系收缩为近似重复，历史全部保留并统一类型 用户要求结合现有文章判断双类型必要性。本地快照有 49 组（35 same_event、14 equivalent）；同原文跨源收录与互补事件报道确有区别，但用户更重视减少重复阅读和规则简单，决定后续只识别近似重复及完全重复，不再追踪仅同一事件。最初提出删除旧事件组/取消主题的方案被用户明确纠正： 全部旧关系转为唯一新类型，成员保留；一句话概述继续保留，旧空值用占位 。用户也明确接受旧事件成员可能不符合新标准。不能再按此前 v4 授权清空数据，也不能用新版重复标签反向证明旧组可用于自动拦截。 实现采用 schema 5 原位改写 kind，而不是重建关系或请求 API 重新判定。保留历史重叠组；新关系仍显式加入稳定组，不合并、不递归推导、不新增重叠归属。所有新组必须有一句话概述；加入已有非空概述组可继承，加入旧空概述组须补充。缺失概述响应失败并保留队列，不允许静默保存占位。概述语义约束继续用 Prompt，不加入复杂句法判断。旧组占位是展示策略，不是让模型学习输出占位。 UI 最终采用中性灰“相关文章”共享信息框，框内先展示去重概述，再展示文章横条；历史多组概述均保留，折叠条数和阅读动作不变。默认 Prompt 升级，精确旧默认可自动迁移，真正自定义保留并向用户提供完整新文本。文章输入前缀保持稳定，可变组状态仍放后缀；首次协议/默认 Prompt 切换需要重新形成缓存。本轮不更改运行中的 Hive，不进行付费模型调用，不提交/推送/发布，需运行新构建才执行真实数据迁移。 2026-09-24：图片挂起请求设定边界并明确等待状态 用户多次遇到前台图片保持半圈不动。截图证实是静态 PNG 的加载占位，而目标 GIF 稍后可以播放；原占位四秒后固定 value=0.72 导致误以为卡死。查明默认图片下载未显式限制响应头/正文等待，重试依赖先抛错，可能永久占用队列。引入共享有界下载层，保留既有缓存，以每请求独立 IOClient 实现真实取消及请求间隔离；保留现有有限重试而不叠加一层无限重试。占位改成文字等待提示。网络缺口已通过可控 HTTP 测试验证，但不宣称已证明每一次历史卡图的具体根因。 2026-09-24：单个静默来源不隐含未分组筛选 在新构建验收中，少数派来源显示 0 篇，而所属 OTHERS 分组及侧栏 8 篇未读均正常。数据未丢失，原因是缺省 groupId 被规范为未分组后与 feedId 取交集。决定让具体 feedId 与分组范围互斥，作用于入口及静默设置更新规范化；不通过重写数据、取消静默或把来源搬到未分组绕过。测试覆盖分组来源切换与未读/全部范围。 决策日志 用一句话主题锚定事件组，取消重叠合并（2026-09-24） 背景：金门大桥演示被收入混合多家模型发布/评测的 72 篇组。旧逻辑先让模型给出文章组，再无条件合并共享成员的事件组；模型看不到已有组及理由，界面递归连通又放大了关联。缺少原始分组记录，具体首个错误连接无法还原，不能只归咎于模型能力或 Prompt。 讨论与决定：用户提出把既有组的主题作为后缀输入，保护长文章前缀，并让后续通过组声明关联。最终选择每篇文章最多一个事件组，跨事件比较只有明确主事件才归入；近似重复独立保留，不触发事件合并。主题只用一句话，允许必要微调，具体性与不逐步扩张由 Prompt 约束，拒绝引入复杂的代码语义规则或长概述。界面用主题框显示，默认三条、统一展开收起；用户随后要求默认展示条数可在设置调整，两端共享持久设置并进入配置备份，不再以图距离决定是否出现折叠。 实现：稳定 A ID 的文章数组在前，new_ids、事件主题目录、当前输入的完整归属映射和协议在后；新组输出 topic，后续明确输出 group_id，程序验证引用与唯一归属，不偷换成文章桥接。修改前主题保存在 topicHistory；主题目录目前发送全部保留事件的短主题，应观察长期 token 成本。用户自定义 Prompt 不再触发旧 N/H schema；历史逐字默认值迁移，真正自定义文本保留且需用户更新为 v4。 数据与验证边界：用户明确不修理旧关系，授权清空队列/关系从头使用。新构建首次初始化迁移时清空关系节点、组和窗口，保留文章、摘要、批次/调用账本及递增序号；写入新时间边界，拒绝跨 epoch 旧结果。当前运行的旧应用未直接写库，新构建尚未运行，重置待生效。Flutter 测试受 Xcode 许可阻塞；未提交、未推送、未发布，本项视觉验收累计到 RELATION-01。 不要回退：不得因共享文章自动合并事件、恢复递归连通展示、把摘要不足作为明确同一原始内容的否决条件，或在用户保存 Prompt 后切回每批变动的 N/H ID。 修复代理动图识别，复用已有播放门控（2026-09-19） 背景：用户反馈运行中的 v2.3.2 高耗电，现场窗口可见但失焦、队列为空，CPU 接近一个核心且持续出帧。日志中的动图登记为零，原生采样却捕获 GIF 解码；用户指出当前少数派文章后，真实正文确认 4 张 GIF 经 Folo 代理包装，旧分类只检查外层地址。无动图文章在窗口可见状态下的 CPU 随后降到 0.0%，支持本轮定位，但不是新代码效果验证。 决策：把动图判断集中到图片服务，精确识别 Folo 根路径代理并安全解码原图参数，复用既有 GIF/APNG 规则；后台预取和两种正文图片入口继续共用缓存服务的分类接口。只修识别缺口，保留代理请求、缓存键、200px 可视区余量、失焦暂停与恢复策略，不新增全局帧率限制或改动文章布局。异常参数编码需退化为不识别，不能因格式错误让正文构建抛异常；回归已证实直接读取 queryParameters 会"
 },
 {
 ...
 },
 {
  ...
  "text": "发布与构建 除非用户明确要求，否则不要创建 release。 用户约定（2026-09-24）：今后用户说“打包”，默认要求完成构建并发布正式 GitHub Release，构成发布授权；不得只触发 workflow_dispatch 产物构建就视为完成。明确说“只构建”“不发布”时按限定范围执行。仍须遵循下述 main 分支、release.sh、版本规则、预检、annotated tag 与发布验证流程。该约定面向后续请求，不自动把此前已触发的仅构建任务升级为发布。 构建发布流程 main 候选提交先由官方 Flutter SDK 完成跨平台预检，成功后云端 prepare_release.py 更新 pubspec 并创建 annotated tag，同一云端流程继续签名和发布 Android APK 与 macOS arm64 产物。 pubspec.yaml X.Y.Z+N 唯一版本来源 scripts/release.sh 仅 main · 触发完整云端流程 annotated tag vX.Y.Z 发布足迹 releases.html 追加 GitHub Actions tag 触发 · Node 24 Actions Android APK 签名对齐 · versionCode macOS arm64 macos-26 runner · SwiftPM GitHub Release release notes 校验 当前发布流程： 唯一正式发布入口：在干净 main 执行 scripts/release.sh <version> --notes-file <file> --push 。本机只推送并 dispatch .github/workflows/publish-release.yml ，确认受理即返回；不用 nohup，不依赖终端或 agent 继续存活。不带 --push 仅做 dry-run，不创建本地 tag。 Publish Release 同一 run 按依赖执行：校验版本/候选 SHA/说明 → reusable Release Preflight（静态分析、测试、Android/macOS 冒烟）→ prepare_release.py 创建版本提交、历史记录、annotated tag → reusable 构建发布 worker → 核验正式 Release 与 APK、arm64 ZIP、appcast 完整上传。 不依赖 GITHUB_TOKEN 推 tag 触发下游工作流，因为 GitHub 会抑制该 push 的递归触发；父工作流直接 uses 调用发布 worker，并传入准确发布 SHA 和 tag、inherit secrets。 internal-release.yml 的手动入口明确命名为 Build Artifacts / Release Worker，只用于明确要求的仅构建；不能把其手动运行当成发布。历史用户推 tag 仍兼容。 全链路串行发布。预检失败不创建版本/tag；预检期间 main 前移会明确失败，重新 dispatch 即可。main 与 tag 原子推送，绝不 force-push。创建 tag 后若构建/上传失败，可重跑：先核对 tag 的父提交、版本与说明完全匹配，复用 tag，不再升构建号。 一般候选是当前 main。补发历史 main 候选时明确传 --candidate <full-sha> ：云端确认属于 main 历史，在候选上增加发布元数据，并同步当前已批准的 .github/workflows 及发布脚本后打 tag；main 单独同步版本与历史，保留后续代码，避免把未授权依赖变更混入补发。发布工作流代码来自触发时 main，应用代码固定为候选；两者不得混淆。GitHub 对历史标签与默认分支不同的工作流要求 workflows 写权限；保持标签工作流与 main 完全一致，避免为了补发增加高权限令牌。应用代码与锁文件仍来自原候选。 “完整流程已受理”不等于“已发布”。按用户约定只确认完整 Publish Release run 已启动并提供链接，不持续等待最终结果；实际成功由末尾 Release/附件核验决定，失败由用户反馈后排查。 本地和 GitHub Actions 统一使用 Flutter 3.47.0 ；项目最低约束为 Flutter >=3.47.0 、Dart ^3.13.0 。升级 SDK 时必须同步检查 pubspec.yaml 、lockfile 和两个工作流中的 Flutter pin，不能只改本机。 GitHub 预检以干净的官方 SDK 为基线，再执行仓库跟踪的 scripts/apply_flutter_patches.sh ；这是发布兼容性与视觉一致性的权威结果。 当前补丁只包含文章选择统一行高、 WidgetSpan.rawText 及其复制映射，不得把 本机 Flutter SDK 中其他未审计实验修改混入发布。补丁固定校验 framework revision，不匹配时必须先审计迁移，不能模糊应用。 macOS 发布包必须保持 arm64。 macOS job 固定使用 macos-26 ARM64 runner。原生侧边栏使用了 macOS 26 SDK 的 NSGlassEffectView ，不要改回默认仍可能选择 Xcode 16 的 macos-latest ，否则 CI 可能在 Swift 编译阶段失败。 macOS 原生插件已迁移为纯 Swift Package Manager。仓库不再保留 macos/Podfile 、 Podfile.lock 或 Xcode Pods 引用； screen_retriever 至少保持 0.2.2 ，该版本修复了其 macOS SwiftPM manifest。若构建再次输出 Running pod install ，应先排查是否误恢复 CocoaPods 集成。 每次发布的候选 SHA、构建号和说明由云端 scripts/prepare_release.py 追加到 docs/agent_handoff/history/releases.html （内嵌 Markdown）；根 AGENT_HANDOFF.md 始终保持为短入口。 依赖维护（2026-09-25） pub get 的 available / newer versions incompatible with dependency constraints 列表是版本提醒，不能等同于解析失败；必须看是否 Got dependencies，以及实际 error。用户原粘贴日志真正阻断运行的是 Xcode 许可未接受，已另行解决。锁文件用于固定可复现依赖，不为了清除提示一键升级所有包。 本次优先提高 dio 最低版本至 ^5.11.1、 flutter_cache_manager 至 ^3.4.5；解析只更新这两项及后者要求的 uuid 4.6.0。缓存管理器修复图片缩放解码错误传递、缓存元数据写入与删除路径，Dio 修复流式背压和请求/拦截器挂起"
 },
 {
 ...
 },
 {
  ...
  "headings": [
   {
    "id": "2026-09-26-手势断言进一步定位mos-为首要对照对象",
    "text": "2026-09-26 手势断言进一步定位：Mos 为首要对照对象"
   },
   {
    "id": "2026-09-26-macos-27-升级后兼容性核验",
    "text": "2026-09-26 macOS 27 升级后兼容性核验"
   }
  ],
  "text": "2026-09-26 手势断言进一步定位：Mos 为首要对照对象 本机 Flutter 3.47.0，发布工作流也固定3.47.0；已安装与本地Release FlutterMacOS的Mach-O UUID相同（4C4C4422-5555-3144-A1B0-38B941BF65AA），文件哈希不同不能直接判定引擎不同，签名也会影响哈希。当前引擎665行对应孤立手势结束时NSAssert（没有内部开始状态，仅允许取消），与既有scrollWheel调用栈相符。 发现 Flutter 官方 issue #155148 ：2024-09-13 已在macOS14.6.1 / Flutter3.24.2报告 Mos 平滑滚动触发同类“Received gesture event with unexpected phase”断言。评论确认普通项目可能只有断言，配置NSApplicationCrashOnExceptions才进一步导致进程崩溃。该issue本次读取仍open、无关联修复PR记录；不能据此承诺升级Flutter已解决，也不能证明每个手势phase异常完全同因。Fourier源码未配置该崩溃选项。 本机只读核对：Mos、LinearMouse和Logi Options+同时运行；Mos全局smooth=true，allowlist=false，应用例外中没有Fourier。LinearMouse对已配置MX Anywhere 3S的平滑滚动明确关闭，但另有滚动参数，所以尚不能排除工具组合影响。未修改这些工具的设置。现有证据优先支持滚动增强工具与Flutter输入状态机兼容边界，而非macOS27独有问题。 用户已确认主要使用触控板，Mos、LinearMouse 与 Logi Options+ 经常同时运行，但实际崩溃频率很低。因此工具组合只是待验证因素，不能认定 Mos 是根因；尚未进行 A/B 对照。用户同意本轮调查告一段落，后续有明确复现时再核对设备、工具启停与断言时间。未修改第三方设置、Flutter 引擎或 SDK，也未宣称问题已修复。 2026-09-26 macOS 27 升级后兼容性核验 用户反馈已在 macOS 27 使用一段时间、表面无问题，要求检查底层。实际系统 macOS 27.0 / 26A428 ，Xcode 27.0 / 27A266a ；安装记录 2026-09-25 05:33:58 UTC（北京时间 13:33:58）。已安装应用 /Applications/Fourier.app 为 2.4.1+40；此次检查时进程已退出，不自行启动或操作真实文章。工作区还含关系 v5、信息框与导航修复，不能把现有应用运行记录等同于这些未提交改动已验证。 验证： xcodebuild -checkFirstLaunchStatus exit 0，当前工作区 flutter build macos --release --no-pub 成功（35.0MB），产物 Mach-O arm64。未安装/启动产物，未推送/发布。Xcode 27 的 x86_64/arm64 配置提示是信息，不是失败。 原生玻璃使用 #available(macOS 26.0, *) ，因此 27 继续走 NSGlassEffectView；WebKit 全屏、低功耗/温度诊断和光标接口也按最低版本保护，没有发现仅匹配“等于26”而使27回退的分支。原生代码构建通过不等于所有菜单、媒体和视觉交互逐项实测。 用户与系统 DiagnosticReports 未找到升级后的 Fourier 崩溃报告。找到的 9月19日崩溃与 9月22日磁盘写入资源报告均属 macOS 26.6.2、Fourier 2.3.2，不能归因于本次升级。磁盘报告为约20小时2.15GB写入告警，单独保留历史线索，未证明当前仍存在。 升级后已有能耗日志共1175条；按采样时 appActive=false 的1005条，CPU中位数0.03%、P95 3.53%，未见普遍持续后台满载证据；前台170条中位数16.85%、P95 64.39%，混合滚动/切页/加载操作，不能等价于静态页空闲耗电。1174条热状态0、1条热状态1。记录是采样窗口且状态为采样时刻，不能据某条隐藏时非零 CPU 推断隐藏后持续渲染，也不据此承诺无泄漏。 发现需保留的异常： 升级后系统日志有4次 FlutterViewController.mm:665 断言（北京时间9月25日18:04:29、18:04:43，9月26日10:09:34、10:31:57），后接 NSInternalInconsistencyException、FlutterMacOS/NSView scrollWheel 调用栈。本机 Flutter 引擎源码对应 Received gesture event with unexpected phase ：收到手势结束但内部没有开始状态。日志原因私有化，源码与调用栈支持手势状态边界判断，但不能单凭它确认是macOS27回归或用户可见故障。没有对应退出崩溃文件，不应声称完全无异常；后续单独追踪滚动/切窗边界，不能直接修改本机 SDK 冒充可复现工程修复。 另有 AppIntents/linkd 4097 注册失败、私有 Metal 警告、CoreAnimation 更新返回 NO；SQLite 缺文件指向系统 /private/var/db/DetachedSignatures ，不是 Fourier 的 Hive 文章库。没有足够证据对应具体功能损坏，不盲目改权限、签名或关闭安全检查。 结论：具备继续使用的构建和运行证据，未发现需要回退系统的阻断问题；保留手势断言作为未关闭问题。此次只做检查和文档记录，未修改兼容性代码。完整398项测试此前已在同一macOS27环境通过；实际媒体、休眠恢复和长时间前台闲置不能据测试结果全部宣告通过。 macOS 说明 macOS 窗口分层 AppKit 原生层与 Flutter 内容层分离：原生玻璃由 sidebarBackdropHost 裁剪，红黄绿自绘按钮转发系统 action，拖动只由 MacOSWindowDragArea 触发。 窗口外框 · 连续圆角 24 · full-size content view sidebarBackdropHost：只裁剪原生玻璃宿主的 24px AppKit 圆角 原生 backdrop 外扩 1px 堵住抗锯齿漏底细缝 AppKit 原生层 NSGlassEffectView(.regular) 侧边栏玻璃 自绘红黄绿 NSControl（转发系统 action） 窗口 isMovable=false · 启动隐藏到首帧 Flutter 内容层 页面 / 侧边栏内容 / 阴影与 0.5px 轮廓 MacOSLayoutMetrics 几何真值 → channel 同步 深浅主题与 NSApp.appearance 同步 拖动区 MacOSWindowDragArea 标题与空白区主动拖动 按钮/输入框不入子树 全屏视频：唯一临时隐藏红黄绿按钮的页"
 },
 {
 ...
 },
 {
  ...
  "headings": [
   {
    "id": "2026-09-26-发布候选-v250",
    "text": "2026-09-26 发布候选 v2.5.0"
   },
   {
    "id": "2026-09-26-macos-窗口圆角收紧",
    "text": "2026-09-26 macOS 窗口圆角收紧"
   },
   {
    "id": "2026-09-26-审核相关文章导航修复待新构建验证",
    "text": "2026-09-26 审核相关文章导航修复（待新构建验证）"
   },
   {
    "id": "2026-09-26-关系-v5本地源码待运行新构建",
    "text": "2026-09-26 关系 v5（本地源码，待运行新构建）"
   }
  ],
  "text": "2026-09-26 发布候选 v2.5.0 本轮整理为关系 v5 与相关文章信息框、审核导航生命周期修复、macOS 外框圆角 18px/侧边栏 12px，以及排障与验收文档。用户已授权分批提交、推送和正式发布；通过完整 Publish Release 云端流程执行预检、版本/tag、Android/macOS 构建及正式 Release 附件核验，确认启动后不持续轮询。关系简化及信息区重构按 Minor 发布，预计构建号 41，由云端生成。以下各阶段“未提交/未发布”描述保留为当时状态，以此发布记录和云端结果为准。 已核实上一正式版 v2.4.1 于 2026-09-25 发布，远端 main 的版本元数据已同步。v2.5.0 尚不能因工作流受理而认定发布完成。旧关系成员/概述保留，真正自定义 Prompt 保留；本机关系 Prompt 已按用户要求移除覆盖值并回归默认。低频触控板断言未修复，后续按复现跟进。 2026-09-26 macOS 窗口圆角收紧 外框 24→18px、侧边栏 18→12px，Flutter 共用布局参数，原生玻璃外框与启动默认值同步。保留连续曲率、间距和按钮位置。详见 Liquid Glass ，待新构建视觉确认；未提交或发布。 2026-09-26 审核相关文章导航修复（待新构建验证） 文章控制器改为所有挂载视图共同管理释放；相关文章导航状态由当前栈观察器维护，返回绑定具体路由并主动调度一帧；审核/时间线旧详情不再跨左侧选择响应；移除动画不覆盖用户后续手动选择。保留原 M 语义与左侧返回锚点，未对用户真实文章执行审核操作。详见 垃圾拦截 ；验证结果与待运行场景分别见验证记录、待办页。本轮未提交、推送或发布。 2026-09-26 关系 v5（本地源码，待运行新构建） 后续只建立“近似重复”一种关系；全部旧关系转换为 equivalent，成员和概述原样保留，旧空概述显示“暂无关系概述”。迁移保留节点、队列、历史、激活边界与账本，不执行 v4 清空流程、不重跑旧文章。新组必填一句话概述，旧空概述组新增成员时补填；缺失响应失败保留任务。历史重叠关系保持，不新增重叠归属或递归扩散。默认/协议已升级，真正自定义 Prompt 需手动替换，完整可复制文本见 翻译与摘要 。本轮尚未修改运行中数据、提交、推送或发布；真实迁移和视觉检查待新构建。 当前状态 最近更新：2026-09-25（各事项的验证时间以对应记录为准）。 2026-09-25 依赖维护第一批：Dio 5.11.1、flutter_cache_manager 3.4.5，以及必需的 uuid 4.6.0；不升级 SDK/图片组件大版本/媒体插件。387 项测试通过，全量分析仅既有 59 项 info，本地网络冒烟及 macOS Release 编译通过；曾捕捉到 Xcode CoreDevice/CoreSimulator 组件版本不一致；用户执行 sudo xcodebuild -runFirstLaunch 后初始化检查与 macOS Release 重建通过，组件报错消失，用户随后确认 flutter run 不再出现大量红字。本轮仅提交推送，不打包、不发布。本轮依赖变更与此前 v2.4.1 候选提交区分，不能因发布日期接近就视为已进入发布。 v2.4.1 曾未完成发布：候选 92ddd9481fea0754d4751c4ec5b054d2a297b028 的预检 36114106555 成功，但本机后台脚本未继续创建 tag，正式发布未触发。不能把当时“预检已启动”解释为发布完成。本轮修复为完整云端 Publish Release，并按原候选补发 v2.4.1（构建号 40），后续 Dio/缓存依赖升级仍只保留 main。完整云端流程首次运行 36118746311 又在 prepare 推送历史工作流时遇 GitHub workflows 权限限制；预检全通过、原子推送未留下 tag。已补修历史候选保留应用代码但同步当前批准的工作流/发布脚本，并补充模拟远端拒绝的回归。新视觉改动尚未收到人工验收反馈。按用户约定确认完整云端流程启动后返回，最终结果由 Actions/Release 确认。 2026-09-25 相关文章主题统一改为列表上方横幅；垃圾拦截正文在摘要上方显示完整拒绝理由，左侧卡片不变。抽取轻量 ArticleInfoCard 与摘要共用外观及可选文本渲染，可见性仍由原审核上下文控制。定向分析、15 项既有文本选择回归通过；本轮分批提交，视觉检查见 READER-INFO-01。 2026-09-25 共享变形选择面板的返回按钮对齐修复已完成（本轮分批提交）：六个入口共用固定锚点、34px 点击范围和 26px 可见圆形；目录不改。三项 widget 回归和定向分析通过，待 MORPH-ALIGN-01 新构建视觉检查；原窗口边缘整体避让保留。 本轮功能提交已推送。用户随后要求补齐正式发布，预检已通过， scripts/release.sh 已创建并推送 v2.4.0 （ 2.4.0+39 ，发布提交 f4a639e ）；正式发布工作流已成功，APK、macOS arm64 ZIP 与签名 appcast 均已上传。代码提交： 7e42c48 目录、 731299a 关系 v4 与展示设置、 402a7d4 失败列表、 2ecbebb 静默来源筛选、 f554d8a 图片等待与超时；此前本地 0abf4a6 为代理 GIF 识别。文档集中记录前因、方案、验证证据与未验证边界。 图片挂起处理已补齐：前台/预取共用有界下载层，响应头 20 秒、无数据 20 秒、总时长 90 秒，超时真实关闭连接并沿用有限重试。四秒后加载圈改文字等待状态，段落内嵌照片也有正文重试流程；用户已运行新构建且未再见加载占位；真实慢网络超时恢复留待后续观察（IMAGE-WAIT-01），未清缓存。 已修复静默分组内点击单来源误显示 0 篇：指定 feed 时不再隐含“未分组”限制；回归已复现并通过修复验证，相关 11 项测试通过。用户重建后已打开少数派目标文章；完整来源/分组切换后续按需观察。 2026-09-24 晚间新 Release 已运行：用户完成目录面板视觉检查；关系 v4 迁移与首两批真实调用已确认，22 组、无重复事件归属。Xcode 许可已解决，9 个相关文件共 96 项测试通过，另有临时隔离设置核验 1 项通过。后续全量 384 项测试通过；新增任务中心测试及临时隔离设置检查 3 项通过。用户已确认失败列表和相关文章显示正常，动图播放/暂停证据及边界见验证记录。 macOS 任务中心摘要/翻译“查看失败”已统一为原玻璃面板内的子页，保留关闭控件与概览滚动返回，窄布局按钮可换行；导航/窄窗长标题回归通过，并修复返回时首帧长度估计导致的约 10px 滚动偏移；用户已确认新构建失败列表显示正常，TASK-UI-01 本轮显示验收关闭。 关系 v4 源码已完成：一句话事件主题、稳定组 ID 显式加入、唯一事件归属、独立近似重复、可配置折叠条数（默认三条）；取消重叠合并与递归扩散。新构建首次运行会按用户授"
 },
 {
  ...
  "headings": [
   ...
   {
    "id": "macos-27-手势断言待定位2026-09-26",
    "text": "macOS 27 手势断言待定位（2026-09-26）"
   },
   {
    "id": "2026-09-26-审核相关文章导航待运行检查",
    "text": "2026-09-26 审核相关文章导航待运行检查"
   },
   {
    "id": "2026-09-26-关系-v5-待运行验收",
    "text": "2026-09-26 关系 v5 待运行验收"
   },
   {
    "id": "操作事项",
    "text": "操作事项"
  ...
  "text": "2026-09-26 macOS 窗口圆角收紧 下次新构建检查浅/深色下窗口四角和侧边栏轮廓，聚焦/失焦及缩放窗口时没有玻璃外溢、锯齿或边缘错位；确认 18px 外框与 12px 侧边栏收紧程度合适。此项为视觉调整，构建不能代替人工验收。 macOS 27 手势断言待定位（2026-09-26） 升级后记录4次 FlutterViewController.mm:665、NSView scrollWheel 路径断言；进一步发现本机Mos全局平滑开启且未排除Fourier，Flutter官方#155148在macOS14已有同类报告，用户确认主要使用触控板且多工具常驻、崩溃低频；尚未做工具开/关对照，本轮调查暂告一段落；暂无对应崩溃或明确用户症状。构建和现有功能使用正常，但此项不能标记已解决。后续核对触控板滚动结束、切窗/隐藏恢复及引擎修复路径；需区分macOS变化与既有Flutter手势边界，保留可复现构建，不直接改本机SDK作为最终方案。证据见macOS平台说明。 2026-09-26 审核相关文章导航待运行检查 代码已修复生命周期与延迟回调缺口；下次集中视觉检查：从审核 A 打开仍在审核列表的 B，再对 B 按 M，确认 B 退出列表并返回 A；换成列表外 B 重复；A→B→A 后逐层 Esc/M；打开关联期间改点左侧另一张卡片，再按 M；移除动画期间改点另一张或 Esc，确认不被后继选择抢回。未读转已读返回，已读恢复未读留在当前页；K/N 不应处理被遮挡的根文章。不可把受控测试通过描述为已复现并消除用户所有偶发现象。 2026-09-26 关系 v5 待运行验收 新构建启动后检查旧关系仍在、统一为近似重复；旧有概述保留，空概述显示“暂无关系概述”，历史重叠组概述不丢失。宽窄窗口与浅深色下，“相关文章”外框与摘要/拒绝理由风格一致，框内概述不另套框，文章横条有左右边距、长句换行正常，展开/收起仍在框内。自定义关系 Prompt 需替换为专题页 v5 文本；新组概述质量与近似重复阈值长期观察。无须重新生成全部历史摘要或清空关系。 待办与搁置事项 这是当前有效的未来事项索引，但不是必须立刻执行的任务列表。 history/archive/ 中的旧“后续计划”只是历史证据；除非已经提炼到本页或当前专题页，否则不能据此直接开工。旧章节的时间顺序和位置通过 history/chronology.md 查询。 操作事项 发布：当前源码版本为 2.4.0+39 。除非用户明确要求，不提前改版本、打 tag 或触发 release；用户说“打包”默认包含正式发布；下一版本号应根据此后的实际改动规模按既有语义化规则决定。 Worktree 清理：本地可能仍有功能 worktree。删除前必须检查，并且只在用户确认后清理。 旧 GitHub Release assets：可能包含历史签名或构建产物；用户可以在未来 release 后手动删除。当前不主动轮换密钥或改写历史。 Android 多方式登录：Google 的精确深链、Session 落地和数据重建已真机确认；仍需验证方式选择 UI、取消、GitHub、Email 错误反馈、Email 正常登录及 TOTP（具备对应账号时）、账号切换确认和本地退出重登。当前 intent filter 只声明精确 host；若仍出现应用选择器或进入官方 Folo，必须停止并重新评估，不能扩大到整个 folo:// 。 Android 误分类按钮：macOS 已上线（ N /右上角旗帜按钮）；Android 是否安装待定。字段与统计语义已随共享模型落库，接入时复用 applyMisclassify / applyFilterReject / applyFilterKeep 与页面级/组件级快捷键模式，并补充本页 features/filter-review.md 的禁用规则。 下次集中视觉与交互验收（持续累计） 2026-09-24 用户暂不方便调试，要求继续修改其他问题，等下次调试时一次性列出所有待检查项。后续 agent 每完成一项相关修改就在此追加或更新，不要求用户逐项即时验收；只有收到明确验证反馈后才标记通过。静态检查不代替视觉验收，也不自动构成发布授权。 READER-INFO-01｜主题横幅与审核理由（2026-09-25，待新构建） ：宽窄窗口下主题均在相关文章上方且同宽；v5 所有关系统一显示概述横幅，旧空概述显示占位；检查历史重叠组的多条概述。2026-09-26 后续调整为“相关文章”共享信息框，概述与文章横条均在框内，概述不再单独套框。垃圾拦截右侧显示完整理由，左侧卡片保持原样；隐藏摘要时理由保留，空理由不留空卡。普通时间线/订阅源详情/相关文章跳转不显示审核理由。检查长理由换行、选中复制、浅深色与摘要外观未退化。 MORPH-ALIGN-01｜共享选择面板返回按钮（2026-09-25，待新构建视觉检查） ：检查时间线范围、订阅源详情范围、排序、静默批量导出、订阅已有分类、内容类型。正常空间下展开/收回图标中心应一致，返回圆形缩小但点击范围不缩小，标题不遮挡按钮；浅深色与变形动画仍需肉眼确认。自动化已覆盖长标题、点击边缘与右上窗口边缘。原有窗口避让可能整体平移面板，不属于本次已消除的标题行偏移，不宣称极边缘绝对同心。 TOC-01｜目录面板（2026-09-24，已验收） ：用户已完成目录视觉检查；层级遮挡和变形动画期间按钮同心位置修复关闭。 TASK-UI-01｜任务中心失败列表（2026-09-24，用户确认显示正常） ：新构建中用户已确认失败列表显示正常；自动化覆盖摘要/翻译入口、窄窗口长标题无溢出及返回滚动位置。本轮显示验收关闭；未逐项人工执行 Android 返回、空状态、失败记录重试等所有分支，不将确认范围扩大。 RELATION-01｜事件主题与相关文章（2026-09-24，用户确认显示正常） ：新构建中用户已确认相关文章显示正常，本轮显示验收关闭。首次迁移及真实重启后保留 22 组已确认；默认条数的持久化与配置导入导出通过隔离测试。后续显式入组、主题更新和长期漂移质量按用户要求留待以后，不阻塞本轮提交。 ARTICLE-01｜Paul Graham 正文（此前遗留，待确认） ：使用包含修复的版本打开《Making Startups Powerful》，检查完整正文、文末脚注、链接和离开后重新打开的显示。 ARTICLE-02｜Appinn 声明清理（此前遗留，待验收） ：使用包含 99c3e54 的构建打开 Wake Me Up 文章，确认注入指令消失且正常正文、4 张图片保留；旧摘要需手动重新生成后再检查，不能仅打开旧摘要判定新清理无效。 SILENT-SCOPE-01｜静默来源空列表（2026-09-24，重建后已打开目标文章，完整切换待验收） ：重新运行后点击“静默 → OTHERS → 少数派”，未读应显示对应文章，全部应含已读目标文章；切回分组及其他来源正常。动图现场结果见 ENERGY-01。 IMAGE-WAIT-01｜图片等待与失败恢复（2026-09-24，新构建未复现，后续观察） ：用户已重建，未找到仍"
 },
 {
  ...
  "headings": [
   {
    "id": "2026-09-26-v250-发布前核对",
    "text": "2026-09-26 v2.5.0 发布前核对"
   },
   {
    "id": "2026-09-26-macos-窗口圆角收紧",
    "text": "2026-09-26 macOS 窗口圆角收紧"
   ...
   {
    "id": "2026-09-26-macos-27-兼容性检查",
    "text": "2026-09-26 macOS 27 兼容性检查"
   },
   {
    "id": "2026-09-26-审核相关文章导航生命周期",
    "text": "2026-09-26 审核相关文章导航生命周期"
   },
   {
    "id": "2026-09-26相关文章中性灰配色",
    "text": "2026-09-26：相关文章中性灰配色"
   },
   {
    "id": "2026-09-26相关文章共享信息框",
    "text": "2026-09-26：相关文章共享信息框"
   },
   {
    "id": "2026-09-26-关系-v5单类型保留历史与概述",
    "text": "2026-09-26 关系 v5：单类型、保留历史与概述"
   },
   {
    "id": "2026-09-25-历史候选发布的工作流权限补修",
    "text": "2026-09-25 历史候选发布的工作流权限补修"
   },
   {
    "id": "2026-09-25-发布中断根因与完整云端流程",
    "text": "2026-09-25 发布中断根因与完整云端流程"
   },
   {
    "id": "2026-09-25-xcode-开发组件初始化复核",
    "text": "2026-09-25 Xcode 开发组件初始化复核"
   },
   {
    "id": "2026-09-25-依赖维护第一批",
    "text": "2026-09-25 依赖维护第一批"
   },
   {
    "id": "2026-09-25-v241-发布授权",
    "text": "2026-09-25 v2.4.1 发布授权"
   },
   {
    "id": "2026-09-25-主题横幅与审核正文理由",
    "text": "2026-09-25 主题横幅与审核正文理由"
   },
   {
    "id": "2026-09-25-共享选择面板返回按钮对齐",
    "text": "2026-09-25 共享选择面板返回按钮对齐"
   },
   {
    "id": "2026-09-24-v240-正式发布",
    "text": "2026-09-24 v2.4.0 正式发布"
   },
   {
    "id": "2026-09-24-新构建显示验收与分批提交",
    "text": "2026-09-24 新构建显示验收与分批提交"
   },
   {
    "id": "2026-09-24-其余检查与任务中心滚动恢复",
    "text": "2026-09-24 其余检查与任务中心滚动恢复"
   },
   {
    "id": "2026-09-24-图片挂起修复验证",
    "text": "2026-09-24 图片挂起修复验证"
   },
   {
    "id": "2026-09-24-动图后续播放与图片占位结论",
    "text": "2026-09-24 动图后续播放与图片占位结论"
   },
   {
    "id": "2026-09-24-动图现场未通过播放验收",
    "text": "2026-09-24 动图现场：未通过播放验收"
   },
   {
    "id": "2026-09-24-静默单来源空列表修复",
    "text": "2026-09-24 静默单来源空列表修复"
   },
   {
    "id": "2026-09-24-新构建运行核验",
    "text": "2026-09-24 新构建运行核验"
   },
   {
    "id": "2026-09-24-任务中心失败列表容器统一",
    "text": "2026-09-24 任务中心失败列表容器统一"
   },
   {
    "id": "2026-09-24-相关文章默认展示条数设置",
    "text": "2026-09-24 相关文章默认展示条数设置"
   },
   {
    "id": "2026-09-24-事件组锚定与关系-v4",
    "text": "2026-09-24 事件组锚定与关系 v4"
   },
   {
    "id": "2026-09-24-macos-目录层级与同心收起按钮",
    "text": "2026-09-24 macOS 目录层级与同心收起按钮"
   },
   {
    "id": "2026-09-19-少数派代理-gif-高耗电",
    "text": "2026-09-19 少数派代理 GIF 高耗电"
   },
   {
    "id": "2026-09-18-appinn-顶层摘要指令遗漏补修",
    "text": "2026-09-18 Appinn 顶层摘要指令遗漏补修"
   },
   {
    "id": "2026-09-18-v232-发布验证",
    "text": "2026-09-18 v2.3.2 发布验证"
   },
   {
    "id": "2026-09-17-paul-graham-旧式正文提取",
    "text": "2026-09-17 Paul Graham 旧式正文提取"
   },
   {
    "id": "2026-09-14-微信图片第三方代理悬挂",
    "text": "2026-09-14 微信图片第三方代理悬挂"
   },
   {
    "id": "2026-09-13-appinn-固定-ai-总结免责声明清理",
    "text": "2026-09-13 Appinn 固定 AI 总结免责声明清理"
   },
   {
    "id": "2026-09-04-本地静默分组",
    "text": "2026-09-04 本地静默分组"
   },
   {
    "id": "2026-09-03-多模态本地传图与错误诊断",
    "text": "2026-09-03 多模态本地传图与错误诊断"
   },
   {
    "id": "同日扩展抽样",
    "text": "同日扩展抽样"
   ...
   {
    "id": "2026-09-12-行内代码关系组转场与图片占位能耗",
    "text": "2026-09-12 行内代码、关系组转场与图片占位能耗"
   },
   {
   ...
   {
    "id": "2026-09-03-普通视频失效分类与有界缓存",
    "text": "2026-09-03 普通视频失效分类与有界缓存"
   },
   {
  ...
  "text": "2026-09-26 v2.5.0 发布前核对 关系/导航功能已通过本轮 398 项全量测试；最终源码静态分析仍为既有 59 项 info，无 warning/error。18/12px 最终圆角配置的 macOS Release 编译通过，文档 59 页及 diff 空白校验通过。功能提交 a76cab0 ，圆角提交 384a118 ，文档随后独立提交。远端 v2.4.1 版本元数据已快进合入，无覆盖远端提交。云端完整发布仍会执行独立跨平台预检；用户已授权 v2.5.0 发布，确认 workflow 启动后返回，不持续等待完成。视觉及真实升级验证边界仍以 pending 页为准。 2026-09-26 macOS 窗口圆角收紧 外框 24→18px、侧边栏 18→12px，Flutter 与原生玻璃裁切同步。定向 Dart 分析无问题，macOS Release 编译通过；文档索引与 59 页校验通过。最终 18/12px 配置已于发布前重新通过 macOS Release 编译（35MB）。此次为样式参数调整，未新增测试或重复全量测试，未安装、启动、提交或发布；浅深色及聚焦/失焦边缘视觉检查留待下次新构建。 2026-09-26 macOS 27 兼容性检查 当前 macOS 27.0 / Xcode 27.0，Xcode 初始化检查成功，工作区 arm64 Release 构建通过，未安装或启动。未见升级后的 Fourier 崩溃文件；已有1175条升级后能耗记录多数后台接近空闲。但发现4次 FlutterViewController.mm:665 / scrollWheel 手势状态断言，尚未归因也未关闭；AppIntents等其他系统日志没有对应功能损坏证据。详细时间、样本统计、旧版资源报告和结论边界见 macOS平台说明 。此项为检查，不是已修复全部兼容性问题。 2026-09-26 审核相关文章导航生命周期 专项 19 项通过，全量 398/398 通过；覆盖真实 Flutter Navigator 多层返回、帧后返回时插入新页面、销毁旧观察器、重复关闭，以及 A→B→A 控制器保留/最后释放、同 ID 新实例保护、列表内/外非选中目标移除、动画期间手动切换或清空选中不被覆盖。导航测试发现静止时仅 addPostFrameCallback 不请求帧会让返回不执行；修复主动 scheduleFrame 后用例通过。控制器生命周期测试用不加载内容的替身，不产生网络请求。 静态检查无 error/warning，仅 59 项既有 info；文档索引/校验通过。未操作用户真实文章的审核/已读状态，未声称已复现用户描述不完整的那一次偶发异常；真实 M/K/N、列表退场动画与左右选中一致性留待新构建集中验收，场景列在 pending 页。未提交、推送、打包或发布。 2026-09-26：相关文章中性灰配色 用户已查看共享信息框，要求降低橙色强调；相关文章外框改为中性灰，标题图标、重复标签和展开操作改为次级文字色，文章标题保持正常正文色。横条底色与 hover 也采用低透明灰色，不改变摘要/拒绝理由。待下次运行确认浅深色下层次与可读性。 2026-09-26：相关文章共享信息框 已将关系区接入 ArticleInfoCard 的结构化内容入口，标题为“相关文章”，概述取消独立底色边框，横条受共享 12px 内边距约束，展开/收起保留在框内。相关文章概述组件的浅深色/窄宽约束测试通过；文章页面、共享信息框与概述组件静态检查无问题。此前 390 项全量测试属于关系 v5 验证，本次样式调整后未重复跑全量。待用户新构建视觉检查：三类信息框风格、横条左右边距、长标题和概述换行、展开/收起及旧空概述占位。尚未提交/推送/发布。 2026-09-26 关系 v5：单类型、保留历史与概述 全量 Flutter 测试 390/390 通过；其中关系 service/worker/topics 共 26 项。覆盖保留历史组的 ID/成员/概述/历史/禁用状态、队列和激活边界，历史重叠归属映射，不递归跨两级显示，新组/显式加入、旧空概述补填、无效概述整批不出队、迁移幂等及旧 epoch 拒绝写回。 在临时 Hive 中对之前只读取得的 49 组真实快照（35 same_event + 14 equivalent） 做额外隔离测试，逐字段比较仅 kind 改为 equivalent；49 组全保留，14 个空概述保留供 UI 占位，重复迁移结果一致。真实内容和临时测试留在忽略的 scratch，不纳入 Git。 概述组件测试覆盖浅深色、280/900 宽约束、长文本换行、旧空值占位、多个历史概述去重与无组无占位。并非用户实际窗口视觉验收。 dart analyze lib test 无 error/warning，仅 59 项既有 info；文档索引/检查通过（59 页、0 warnings），diff 无空白错误。 没有改写运行中的 Hive，没有付费模型调用，没有提交、推送、打包或发布。真实迁移需运行新构建；新 Prompt 判断质量、真实窗口概述横幅/占位及旧重叠组展示留待使用确认。自定义 Prompt 不覆盖，需使用专题页提供的完整 v5 文本替换；精确旧默认自动升级。 2026-09-25 历史候选发布的工作流权限补修 Publish Release 36118746311 的静态检查、测试、Android/macOS 冒烟均成功；prepare 原子推送失败，GitHub 明确拒绝历史 v2.4.1 标签中与默认分支不同的 internal-release.yml（GITHUB_TOKEN 没有 workflows 写权限）。远端 main 未前移且没有 v2.4.1 tag，发布 worker 被跳过。这是新增发布流程遗漏的权限边界，不是应用构建失败。 历史候选 worktree 现在只同步当前 main 已批准的 .github/workflows、release.sh、prepare_release.py 和其回归脚本，再生成发布元数据；应用及依赖仍来自 92ddd94。无需扩大令牌权限，不改变原发布范围。测试加入远端 pre-receive hook 拒绝工作流树差异，包含新增/修改/删除工作流场景：修复前 4 项中历史候选用例失败，修复后全部通过，并断言应用代码仍为原候选、工作流树与 main 相同。actionlint 与 diff 检查通过。提交修复后重新 dispatch 同一 v2.4.1 候选，不在旧工作流定义上盲目 rerun。 2026-09-25 发布中断根因与完整云端流程 用户再次发现 v2.4.1 没有 Release。查证预检 36114106555 对 92ddd94 成功，最新 Release 仍 v2.4.0，既无 v2.4.1 tag 也无正式发布 run。后台日志停在推送候选；无法确认进程具体终止原因，但原链路依赖本机等待后创建 tag，预检不会自己发布。此前将 nohup 后台启动等同于可靠交接是执行错误，文档允许该方式也是缺陷。 修复：release."
 },
 {
  "path": "index.html",
