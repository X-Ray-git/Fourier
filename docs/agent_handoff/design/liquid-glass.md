...
- Android 主时间线的紧凑 switch 使用 `55 x 29` 轨道、`40px` 滑块、`48px` 透明点击区以及 `0.6px / 55%` 外框。该参数适应移动端密度，不要覆盖 macOS 的 `58/42/1px` 参数。
- Android 固定浮动导航使用单个真实玻璃表面，选中态以橙色前景/指示层表达；当前几何为边距 `12px`、高度 `56px`、半径 `28px`。其外阴影路径必须复用同一 `28px` 半径并裁掉组件内部，只向外扩散。不要用不透明底板或轮廓线补浅色模式可见性。
