...
后果：历史文章重新构建后也会得到紧凑作者区，多作者内容不再被拆成多张全宽图片；普通正文图片、其他来源圆形图片和作者姓名保持不变。后续来源兼容继续集中在该层，以稳定来源标记、域名或明确语义为触发条件并提供反向测试；不要加载整套上游 CSS、解析任意 Tailwind 类、按文章标题硬编码，或退回“所有圆形/小图都是头像”的宽泛启发式。
