...
- macOS 原生菜单已从 Flutter 模板收敛为 `Auto Folo / 编辑 / 显示 / 文章 / 窗口 / 帮助`。业务 Undo/Redo 使用 50 项双栈，`Cmd+Z`/`Shift+Cmd+Z` 可连续处理标为已读及垃圾拦截保留/移除，菜单按 `撤销“动作” · 《截断标题…》` 显示下一项；输入框保留文本 Undo/Redo，scope 变化清空业务栈。`Cmd+W` 已恢复为与红色按钮一致的隐藏窗口行为。用户已完成菜单、动作名称和窗口关闭的视觉/交互检查。
...
- 合并时又把 Android 文章范围改为圆形按钮 + 玻璃选择面板，并把普通/垃圾拦截卡片字级统一为 `14/12px`；这两项以及横滑圆角修正尚需用户在真机做最终视觉检查。
