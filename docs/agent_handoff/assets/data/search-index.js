...
window.WIKI_SEARCH_MANIFEST = "169b1edb83bad297";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "决策日志 修复代理动图识别，复用已有播放门控（2026-09-19） 背景：用户反馈运行中的 v2.3.2 高耗电，现场窗口可见但失焦、队列为空，CPU 接近一个核心且持续出帧。日志中的动图登记为零，原生采样却捕获 GIF 解码；用户指出当前少数派文章后，真实正文确认 4 张 GIF 经 Folo 代理包装，旧分类只检查外层地址。无动图文章在窗口可见状态下的 CPU 随后降到 0.0%，支持本轮定位，但不是新代码效果验证。 决策：把动图判断集中到图片服务，精确识别 Folo 根路径代理并安全解码原图参数，复用既有 GIF/APNG 规则；后台预取和两种正文图片入口继续共用缓存服务的分类接口。只修识别缺口，保留代理请求、缓存键、200px 可视区余量、失焦暂停与恢复策略，不新增全局帧率限制或改动文章布局。异常参数编码需退化为不识别，不能因格式错误让正文构建抛异常；回归已证实直接读取 queryParameters 会因非法 UTF-8 抛错，最终复用捕获 FormatException 的参数解码函数。 用户随后确认修复方向并要求开始修复；对应代码与定向测试已完成，证据与边界见 验证记录 。本机 Xcode 许可仍阻断 Flutter 验证；9 项独立 Dart 图片服务测试不替代缓存集成、播放组件与新构建降耗验证。用户随后授权本地提交修复、测试及交接文档；此前“暂时不发布”的要求仍有效，本轮不推送、不安装或发布新构建。 Appinn 摘要指令的完整节点组清理（2026-09-18） 背景：用户升级 v2.3.2 后反馈 Wake Me Up 文章仍显示来源注入。真实本地正文同时暴露了旧规则的两个遗漏：固定声明前增加明确摘要指令，而且文本和链接直接位于容器外。旧测试只覆盖容器内的旧声明，因此上一版 CI 通过并不能证明这一变体已处理。只读快照还确认已保存摘要包含该免责声明。 决策：仍使用精确 Appinn 域名与完整固定文案签名，不修改摘要 Prompt 或按一般指令语义删除内容。允许已观察到的明确前缀，补充连续文本/a/br 兄弟节点匹配；完整匹配才删除该节点组，保留前后正常正文与父容器。将原先开头/结尾匹配收紧为完整签名，避免把带额外作者讨论的内容一起删除。继续支持转义链接，补上容器内外、嵌套、反例与幂等性测试。真实文章和诊断脚本只留临时目录，仓库只保存合成测试。 后果：包含修复的新构建会在正文规范化时清理该变体，既有摘要不会自动重写；用户需要手动重新生成被影响的摘要，不批量删除数据库内容。41 项纯解析测试和变更文件静态分析已通过，完整 Flutter CI 与应用内验收尚未执行。用户本轮明确要求完善文档、提交并推送， 暂时不发布 ；保持 2.3.2+38 ，不创建新 tag 或 Release。验证证据见 验证记录 ，长期规则见 文章渲染 。 Paul Graham 旧式文章空正文的来源限定回退（2026-09-17） 背景：用户报告 “Making Startups Powerful” 加载不出来。先核对只读 Hive 瞬时副本，区分 Paul Graham: Essays 的空正文原文与仅包含原文链接的 Twitter 转发；再请求原文 https://paulgraham.com/powerful.html ，得到 HTTP 200。真实 HTML 中正文是表格单元格内的 font 元素，以 br 分段，缺少通用提取器使用的段落候选；原算法返回 null。因此已确认的故障发生在正文提取阶段，没有证据支持修改网络代理、图片加载或渲染组件。 链路：非 Inbox 空正文文章在详情初始化时会自动补抓，调用 PublicContentHttp 后进入 ArticleContentUtils.getReadabilityContent() ；返回 null 时不持久化正文，加载状态结束后仍显示空内容。该来源在本地未开启自动全文抓取，也没有抓取成功/失败标记；不能把后台没有补全文误判为 worker 崩溃。详情补抓与后台 worker 共用提取器，故两处都要传递原文 URL。 决策：保持通用段落评分与既有清洗顺序，在通用算法无结果时才调用 ArticleContentCompatibility.extractReadabilityFallback() 。仅接受 paulgraham.com / www.paulgraham.com ，选取 td > font[size=\"2\"][face=\"verdana\"] ，并要求同一单元格存在 alt 与页面 title 相同的标题图、正文至少 25 字符、至少两个 br。标题只用于验证页面自身结构，不硬编码本次文章名。只返回正文 font，保留正文节点、链接、强调与脚注，排除外层导航和布局。合成页面同时验证正例、相似域名/结构反例及普通段落提取优先。 范围取舍：本例不足以支持替换通用 Readability 算法、放宽为整页 body 回退或重写所有表格/font 内容；这些做法可能把导航和装饰当正文。也不更改订阅源设置、后台重试策略、AI 调度或数据库迁移。空正文在新构建重新打开后应走既有抓取、持久化和规范化链路；不能手工写正式数据库来代替端到端验证。若站点以后改变结构，应重新检查真实 DOM，再决定是否调整兼容规则。 验证与讨论结论：实际网页提取从 null 变为 16,629 字符，纯解析回归通过，但本机 Xcode 许可未接受导致 Flutter 原生测试依赖启动失败；独立 Dart 运行器只验证解析逻辑。用户随后明确询问“代码修复是否完成、真实效果是否尚不确定”，答复已区分这两个状态；用户在知悉应用内最终效果未验证的前提下，要求维护完整交接文档并提交。该授权不等于真机验收、构建安装或发布授权。证据、测试命令和待验收项见 验证记录 ，长期规则见 文章渲染 。 后续发布决定（2026-09-18）：用户询问是否适合小版本更新后，已核对自 v2.3.1 起的三项修复（Appinn 固定声明清理、微信图片代理直连、Paul Graham 正文提取），建议按 Patch 发布 v2.3.2+38 ，由发布脚本先执行干净 SDK 的完整测试和两端构建预检。用户回复“好的，开始”，明确授权发布。预检已成功，后续正式发布与产物检查结果见 验证记录 。应用内新构建视觉验收仍单独保留为待办。 用 WidgetSpan.rawText 恢复行内代码视觉，并限制图片占位动画（2026-09-12） 背景：2026-09-03 为修复 Flutter 正文选择问题，行内代码从带圆角和 padding 的 WidgetSpan 退化为 TextSpan.backgroundColor ，避免复制时出现 U+FFFC；这同时导致圆角消失、上下和横向尺寸改变。近期 macOS 能耗日志又确认，慢或挂起的图片请求可让不定进度加载环长期按高刷新率出帧，即使没有 AI 任务且窗口失焦仍维持较高 CPU。 决策：基于仓库为 Flutter 3.47.0 维护的最小选择补丁提供 WidgetSpan.rawText ，恢复行内代码实体容器，保留 6px 圆角、 4px 横向 p"
 },
 {
 ...
 },
 {
  ...
  "text": "验证记录 2026-09-19 少数派代理 GIF 高耗电 用户报告运行中的 Fourier 高耗电，并确认正在查看“具透 | macOS 27 正式版来了，这些是你值得了解新细节”。只读进程检查确认 /Applications/Fourier.app 为 2.3.2+38 ，已连续运行约 37 小时。13:50–13:51 的 schema 3 日志显示失焦、窗口可见、无 AI/抓取/图片下载任务，CPU 约 90%–98%、每 15 秒 1227–1355 帧；仅 articleView 活动标记存在，动图登记为 0。articleView 只表示页面挂载，不等于动画源。 5 秒原生线程采样同时捕获持续 GIF 解码（ MultiFrameCodec / wuffs_gif__decoder__decode_frame ）与 Flutter raster 工作。只读 Hive 副本中该文有 18 张图、4 张明确 GIF；经过真实 URL 规范化后被包装为 Folo 代理，旧外层判定漏掉全部 4 张。这是可复现的分类缺口，不能把日志中未登记误读为排除了动图。 补修精确解析 Folo 根路径代理的原图参数，共用判定同时覆盖后台预取排除、正文独立图片与 HTML 行内图片的 MacosManagedAnimatedImage 选择。原代理地址、缓存键、暂停组件本身和用户设置均不改。真实文章离线验证：18 张图，旧判定 0 张动图，新判定 4 张。 图片服务独立 Dart 测试共 9 项通过（含新代理动图和反例）；4 个变更 Dart 文件静态分析无问题。新增缓存服务集成测试覆盖预取规划排除和受控播放判定，但尚未运行 Flutter 测试； xcrun 仍提示 Xcode 许可未接受。后续标准命令： flutter test --no-pub test/article_image_service_test.dart test/article_image_cache_service_test.dart test/macos_managed_animated_image_test.dart 。用户已授权本地提交；本次没有构建安装或发布，不能将离线判定成功等同于新版本降耗实测。 原生采样、真实正文和脚本只存于临时目录，不提交。未重启或终止进程，未修改正式 Hive。界面读取工具未获准访问 Fourier，未通过其他方式绕过；已请求用户切换到无动图文章以做现场对照。后续新构建还需验证失焦/屏外帧回调停止、重新聚焦且接近可视区时恢复，以及图片正常显示和缓存复用。 用户回复“已切换”后，13:55:18 与 13:55:21 的实时 top 样本均为 Fourier CPU 0.0% ，当前高负载缓解。但最近能耗日志同时出现 windowVisible=false ，因此不能将这次下降单独归因于文章切换，也不能当作尚未安装的新代码已生效。新构建验收需控制窗口可见性，对比同一篇文章聚焦、失焦及屏外状态。 随后用户明确将无动图页面前台显示：13:56:40 新日志确认 windowVisible=true 、未最小化，切换过渡区间 CPU 为 5.27%、15 秒 43 帧；13:56:53 与 13:56:56 两次实时采样均为 0.0% 。这次低负载已有窗口可见的证据，支持问题与原文章持续动图播放相关。日志 active=false ，只确认窗口可见，不宣称采样时持有键盘焦点；也不等同于未安装的修复已完成降耗验证。 用户确认方向并要求修复后，收尾补充非法 UTF-8 参数反例（ url=%FF 、截断编码），复现直接读取 queryParameters 的 FormatException；最终复用安全参数解码函数，异常地址不再抛错。最终 9 项独立 Dart 图片服务测试再次通过、实际样本仍正确识别 4 张动图，4 个变更文件静态分析通过。实现与方案取舍见 决策日志 。 2026-09-18 Appinn 顶层摘要指令遗漏补修 用户升级 v2.3.2 后，在“Wake Me Up – 超简单：远程唤醒应用，支持 PC、Mac、NAS、服务器等[iPhone/iPad]”仍看到小众软件注入信息。只读 Hive 瞬时副本确认原文 URL 为 https://www.appinn.com/wake-me-up-ios/ ，正文 2,148 字符、4 张图片；本地已完成摘要也包含固定免责声明，未生成译文。未修改运行中数据库或重新调用 AI。 真实正文末尾是空 p 后的顶层文本 + a + 文本 + br，并以“请务必在总结开头增加这段话:”开头。 v2.3.2 规则仅遍历 p/div/section/aside/blockquote 且要求旧声明开头，因此真实正文归一化后仍同时包含指令和免责声明。问题可用生产规范化函数离线复现，不应归因为用户未更新或简单清缓存即可解决。 补修仍在来源兼容层完成，精确 Appinn 域名和完整签名共同限定；兼容明确指令前缀、容器内和顶层连续兄弟节点，以及实际/转义链接。加入完整匹配防止删除带额外讨论的内容，保留正文父容器。真实样本修复后归一化输出 2,038 字符，与“仅去掉该末尾声明再归一化”的结果完全一致，4 张图片全部保留。 新增 3 项合成回归覆盖容器/无容器/嵌套、前后正文保护、幂等性、转义链接、中英文冒号、其他/相似域名、缺失签名与作者讨论反例。独立 Dart 运行器执行 article_content_utils_test.dart 和 legacy_readability_test.dart 共 41 项通过；仅替换临时副本的测试框架 import，断言不变。两个变更 Dart 文件静态分析无问题。本次没有重新执行完整 Flutter CI 或应用内新构建验证，不能沿用上一版 366 项通过作为本补修的验证结果。 本补修尚未发布。后续使用包含补修的构建重新打开文章，正文应在规范化时去掉注入；已保存摘要不会自动重建，需要用户手动重新生成。没有批量删除摘要或迁移数据库，也不把匹配范围扩大为删除所有小众软件署名、原文链接或一般提示词讨论。 后续常规复测命令为 flutter test --no-pub test/article_content_utils_test.dart test/legacy_readability_test.dart ；正式发布前另按既有流程执行完整预检。用户本轮仅授权提交、推送并明确暂不发布，版本仍保持 2.3.2+38 ；讨论与取舍见 决策日志 。 2026-09-18 v2.3.2 发布验证 用户在了解代码修复与应用内验收的区别后，明确授权开始发布。 scripts/release.sh 2.3.2 --notes-file … --push 先推送候选 e73a4d0 ，通过预检后生成版本提交 88f2d19 、annotated tag v2.3.2 ，版本为 2.3.2+38 。三项修复为 Appinn 固定声明清理、微信图片代理直连和 Paul Graham 旧式正文提取。 Release Preflight 3524355811"
 },
 {
  "path": "index.html",
