...
window.WIKI_SEARCH_MANIFEST = "052c9b5564543db9";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "验证记录 2026-09-14 微信图片第三方代理悬挂 用户指定文章“GPT-6 Astra开放Plus！20美元只能拿它干活，不能聊天”的只读 Hive 快照显示 13 个图片标签、8 个预取唯一图片；其中一张已登记文章级 cache key，但缓存数据库无文件、failed registry 也无记录，符合下载 Future 悬挂而非明确失败。正文加载环按既有节能规则在 4 秒后停止旋转，因此最终表现为不动的进度弧。 该文正文图均经 img2.jintiankansha.me/get?src=http://mmbiz.qpic.cn/... 中转。带应用请求头实测代理可返回图片，但代表样本约 4.5 秒；同一微信 CDN HTTPS 原图约 0.3 秒，缺失样本直连返回有效 1080x638 PNG。修复只对精确代理 host/path 且嵌套 host 为 mmbiz.qpic.cn 的地址解包，其他来源和相似代理反例保持不变。 article_image_service_test 覆盖真实形态、非微信嵌套来源、相似代理 host 和完整编码查询参数；图片服务、缓存、成功通知及重试定向测试共 19 项通过。诊断脚本和下载样本只存在于临时目录，未写入仓库；正式数据库仅做瞬时副本读取，没有修改。 新版本会生成原图直连对应的新 cache key，无需迁移旧缓存。若其他来源以后仍出现“已登记但既未成功也未失败”的请求，应单独评估缓存下载层的可取消超时，不能仅给 Future 添加不取消底层请求的表面 timeout，也不应恢复无限旋转加载环。 2026-09-13 Appinn 固定 AI 总结免责声明清理 小众软件会在正文中注入一段面向第三方 AI 检索场景的固定总结免责声明；Fourier 用户已经明确知道摘要由模型生成，因此该段在阅读、摘要、翻译和导出链路中属于重复噪声。本轮将规则放入既有 ArticleContentCompatibility 来源兼容层，没有修改通用 HTML 清洗器、Prompt 或数据库原始正文。 规则以 appinn.com / www.appinn.com 原文域名和完整固定文案签名双重门禁，允许内部文章链接变化；同时兼容正常 <a> 与被转义为文本的链接。不得把规则扩展为按“AI”“观点和立场”或免责声明语义做通用删除。 test/article_content_utils_test.dart 新增 4 项定向覆盖：正常链接、转义链接、其他来源反例和 Appinn 相似但不完整正文反例。定向测试 35 项、全量 flutter test --no-pub 358 项通过； dart analyze lib test 无 error/warning，仅有 59 项既存 Liquid Glass prefer_initializing_formals info；文档 59 页检查和 git diff --check 通过。 已保存的旧摘要不会自动重建；手动重新生成或后续新任务会使用清理后的输入。原始抓取内容不迁移，运行时规范化即可让历史正文显示和导出路径生效。 2026-09-04 本地静默分组 本轮实现的是静默订阅源分组，不是前一轮多模态修复。最终平台范围：macOS 保留组内批量导出，Android 仅管理/浏览，没有文章勾选和后续批量动作；配置备份两端都支持。后续移除了重复的“全部静默”范围、macOS 菜单项与 Cmd+0 ，未指定组的旧调用归一到未分组。 flutter test --no-pub ：339 项通过。新分组相关 14 项覆盖旧静默开关、稳定 ID 改名、分组删除保留静默、取消静默清归属、重名/保留名、排序边界、损坏定义不读旧缓存、原 Folo 分类不变、各范围共用筛选、导出处理中的 scope 锁、配置完整替换/旧版回退，以及弹窗取消与名称校验。 dart analyze lib test ：无 error/warning，59 项现存 prefer_initializing_formals info。Android Debug APK 与 macOS Debug .app 均编译通过；Gradle/AGP/Kotlin 的未来最低版本提醒与视频插件的 API 弃用警告仍在，不在本轮升级构建链。 测试全部使用隔离临时 Hive，没有修改正在运行的数据库，未启动正式应用，也未新增 API 请求或修改配置文件中的个人设置。 待用户真机检查：两端新建/改名/排序/移组，删除非空组后继续静默、取消静默后回到原 Folo 分类；macOS 组名/箭头独立行为、组间切换清空勾选、勾选时仍可阅读、组内 Markdown 与撤销；Android 分组跳转/系统返回且无批量入口。自动化通过不等同于两端视觉验收完成。 2026-09-03 多模态本地传图与错误诊断 证据：本地历史账本中视觉请求共 278 次，其中 252 次 HTTP 400；包含 Twitter 图片的 250 次请求均失败。对同一图片进行 URL/Base64 对照，URL 返回服务端下载失败，Base64 能正常识别，不能据此归因于摘要 Prompt 或模型能力不足。 真实服务验证：在隔离 Hive 中复制当前配置，使用现有私有业务 Prompt，对 CursorBench 单图和 a16z 双图分别调用实际 SummaryService / ArticleFilterService 。测试仅将输出上限设为 4096、自动重试设为 0，并替换网络/图片文件加载入口以隔离测试环境，不改正式默认值。8 次请求（4 文本 + 4 视觉）均成功且 finish_reason=stop ；摘要可读取图中具体信息，两项过滤请求也能完成视觉判断。测试替换了图片下载入口，复用缓存的生产路径未在真机新构建上运行验证。 消耗：服务链路 14,194 tokens，加上此前对照测试的 1,791，共 15,985，低于用户授权的 80,000。HTTP 400 对照未返回 usage，不计入已知消费。私有 Prompt、过滤偏好与完整响应仅留在本机隔离测试资料，不写入文档；已删除临时 setting.hive 副本，没有覆盖运行中数据库。 自动验证： flutter test --no-pub 共 325 项通过；覆盖懒加载、顺序/格式识别、空文件/超限、失败降级、重试编码复用、错误响应三种表示的解析及脱敏。 dart analyze lib test 无 error/warning，仅有既存 info。正式应用未启动，未进行 Android/macOS 新构建真机验证。 后续检查：运行含本改动的版本后，手动重新生成此前降级的图片文章摘要，确认内容含图中事实；在新 llmUsageEvents 中核对视觉请求成功率。已有 done 摘要不自动重建，模型是否触发转交仍由现有内置协议判定。单次 API 成功不代表所有源、图片格式和大图都可用。 同日扩展抽样 追加 7 篇真实文章，累计 9 篇：微博 Blender 三图（含中文技术说明截图）、Paul Graham 英文邮件截图、微信图片文章、微信短摘要"
 },
 {
  "path": "index.html",
