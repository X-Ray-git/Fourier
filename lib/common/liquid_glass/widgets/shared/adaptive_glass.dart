...
class AdaptiveGlass extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final baseSettings = (!useOwnLayer && inherited != null)
        ? inherited.settings
        : settings;
    // ---- MINIMAL FAST-PATH ---------------------------------------------------
    ...
    if (!canUsePremiumShader) {
      ...
      if (skipNormalization) {
        ...
        normalizedSettings = baseSettings.copyWith(
          thickness: (baseSettings.effectiveThickness * 0.4).clamp(
            0.0,
            double.infinity,
          ),
          lightIntensity: (baseSettings.effectiveLightIntensity * 0.6).clamp(
            0.0,
            10.0,
          ),
          glassColor: baseSettings.glassColor.withValues(
    ...
    if (effectiveUseOwnLayer) {
      ...
      return PremiumGlassTracker(child: premium);
    } else {
