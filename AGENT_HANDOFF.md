...
## 163. 文章详情状态与正文渲染细节修复（2026-07-07）
...
## 164. macOS 时间线未读/全部切换与系统红绿灯修复（2026-07-07）
...
- 后续与用户再次确认：已读文章入口/页面能力仍然保留；这里“不需要已读”只指主时间线、按分类过滤、按订阅源过滤时，中间栏顶端的快速过滤器不出现 `已读` 选项。
...
## 165. macOS 时间线排序、同步动画与调试日志收口（2026-07-07）
...
- 合并回 main 前再次确认：排序只需要作用于当前本地已加载文章集合，不要求远端全量排序。
- 首次按长文/短文排序会有一次内容长度估算成本；用户当前未观察到明显性能问题，因此先合入并后续实机观察。
...
- 未 tag / release。当前只是合并功能分支，后续若用户确认可发布，再按 `scripts/release.sh` 流程推进。
...
- 本节记录的是 `codex/auto-folo-1` worktree 合并进 main 的上下文。
- 相关文件包括：
  - `lib/http/init.dart`
  - `lib/pages/article/article_page.dart`
  - `lib/pages/settings/settings_page.dart`
  ...
  - `AGENT_HANDOFF.md`
- 合并时还需要和第 164 节的 macOS `未读 / 全部` 二段控件共同存在：
  - header 左侧保留 `未读 / 全部` 快速过滤；
  - 排序按钮放在同步按钮左侧；
  - 已读文章入口/页面仍保留，但 header 快速过滤器不显示 `已读`。
