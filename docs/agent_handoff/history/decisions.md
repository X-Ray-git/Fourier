...
后果：设置页不再存在含义模糊的 LLM 整卡“保存此参数”，普通数字项也不再排列大量难以对齐的保存按钮。macOS 外观 segmented 独占一行，避免和结构不同的下拉框强行等高；服务认证底部使用紧凑的“测试连接 + 保存认证”按钮行，Prompt 操作继续在容器底部右对齐。测试连接使用当前输入且不落盘：Folo 调只读订阅接口，DeepSeek 调需认证的只读模型列表，不产生推理费用。离散参数持久化后从下一次相关请求生效，外观等有运行时监听的设置仍即时反映。格式无效或写入失败时恢复上一次成功持久化的配置并提示用户。
