...
- 2026-07-29 YouTube SABR 首选播放器尚待用户实机验证。macOS 与 Android
  ...
  桥接。
  Bilibili 官方站外播放器不应受影响。macOS Release 还需确认 sandbox 下
  loopback server 可以绑定；Android 需确认只放行 localhost 明文而外部
  HTTPS 行为不变。
...
- Bilibili 正文视频：严格 URL 解析、readability 白名单、懒加载 widget 与完整 Flutter 测试均已通过，用户已允许提交；对话中尚未留下 macOS/Android 实际播放、控制栏和系统全屏的明确逐项反馈。发布前应至少在一篇“本周看什么”中验证点击播放、同篇多视频不预加载和全屏退出。
