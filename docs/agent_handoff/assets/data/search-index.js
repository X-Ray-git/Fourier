...
window.WIKI_SEARCH_MANIFEST = "2c09d8085d9e3a12";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "验证记录 2026-09-18 Appinn 顶层摘要指令遗漏补修 用户升级 v2.3.2 后，在“Wake Me Up – 超简单：远程唤醒应用，支持 PC、Mac、NAS、服务器等[iPhone/iPad]”仍看到小众软件注入信息。只读 Hive 瞬时副本确认原文 URL 为 https://www.appinn.com/wake-me-up-ios/ ，正文 2,148 字符、4 张图片；本地已完成摘要也包含固定免责声明，未生成译文。未修改运行中数据库或重新调用 AI。 真实正文末尾是空 p 后的顶层文本 + a + 文本 + br，并以“请务必在总结开头增加这段话:”开头。 v2.3.2 规则仅遍历 p/div/section/aside/blockquote 且要求旧声明开头，因此真实正文归一化后仍同时包含指令和免责声明。问题可用生产规范化函数离线复现，不应归因为用户未更新或简单清缓存即可解决。 补修仍在来源兼容层完成，精确 Appinn 域名和完整签名共同限定；兼容明确指令前缀、容器内和顶层连续兄弟节点，以及实际/转义链接。加入完整匹配防止删除带额外讨论的内容，保留正文父容器。真实样本修复后归一化输出 2,038 字符，与“仅去掉该末尾声明再归一化”的结果完全一致，4 张图片全部保留。 新增 3 项合成回归覆盖容器/无容器/嵌套、前后正文保护、幂等性、转义链接、中英文冒号、其他/相似域名、缺失签名与作者讨论反例。独立 Dart 运行器执行 article_content_utils_test.dart 和 legacy_readability_test.dart 共 41 项通过；仅替换临时副本的测试框架 import，断言不变。两个变更 Dart 文件静态分析无问题。本次没有重新执行完整 Flutter CI 或应用内新构建验证，不能沿用上一版 366 项通过作为本补修的验证结果。 本补修尚未发布。后续使用包含补修的构建重新打开文章，正文应在规范化时去掉注入；已保存摘要不会自动重建，需要用户手动重新生成。没有批量删除摘要或迁移数据库，也不把匹配范围扩大为删除所有小众软件署名、原文链接或一般提示词讨论。 后续常规复测命令为 flutter test --no-pub test/article_content_utils_test.dart test/legacy_readability_test.dart ；正式发布前另按既有流程执行完整预检。用户本轮仅授权提交、推送并明确暂不发布，版本仍保持 2.3.2+38 ；讨论与取舍见 决策日志 。 2026-09-18 v2.3.2 发布验证 用户在了解代码修复与应用内验收的区别后，明确授权开始发布。 scripts/release.sh 2.3.2 --notes-file … --push 先推送候选 e73a4d0 ，通过预检后生成版本提交 88f2d19 、annotated tag v2.3.2 ，版本为 2.3.2+38 。三项修复为 Appinn 固定声明清理、微信图片代理直连和 Paul Graham 旧式正文提取。 Release Preflight 35243558117 成功：干净官方 Flutter 3.47.0 加仓库最小补丁；Linux 完整测试 363 项通过，3 项 macOS 专属 tooltip 测试按平台跳过；静态分析无 error/warning，59 项既有 info；Android Debug 和 macOS arm64 Release 冒烟构建均通过。本轮 CI 已补齐此前受本机 Xcode 许可阻断的 Flutter 验证，不必继续把“只跑过独立 Dart 解析测试”作为当前结论。 正式发布流程 35244343946 全部成功；macOS 完整测试 366 项通过，发布包主程序确认为 Mach-O 64-bit executable arm64 。Android 签名 APK、macOS ZIP 和 Sparkle 签名 appcast 均构建并上传成功。 v2.3.2 Release 于北京时间 2026-09-18 00:15:35 正式发布，非 draft、非 prerelease。三个附件为 Fourier-android-v2.3.2.apk （70,089,553 字节）、 Fourier-macOS-arm64-v2.3.2.zip （12,873,620 字节）和 appcast.xml （1,862 字节），均有 GitHub 提供的 SHA-256 digest。 已从实际 releases/latest/download/appcast.xml 入口下载清单，文件 SHA-256 与 Release 附件 digest 一致，版本字段为 2.3.2 / 38 ，enclosure URL 与 ZIP 附件地址一致、长度相符且含 Ed25519 签名字段。签名生成由成功的 CI 任务完成；本地没有重新下载全部安装包或运行 Sparkle 安装来独立验证签名与升级链路。 未在用户设备安装或启动新构建，文章实际阅读排版、链接/脚注交互和应用内更新安装仍待用户验收；CI 通过和产物发布不能替代这部分验证。空正文文章升级后重新打开，按原有链路重新抓取；不改正式 Hive、不批量重做旧摘要。 2026-09-17 Paul Graham 旧式正文提取 用户指定的 “Making Startups Powerful” 在 Paul Graham: Essays 本地记录中正文为空，未生成翻译或摘要；同名 Twitter 转发是独立条目，不能混淆。仅对 Hive 瞬时副本读取，未修改正式数据库、设置或文章状态。 原网页返回 HTTP 200、24,270 字节；真实 DOM 没有 p/blockquote/article/div > text 候选，正文位于旧式 font 元素并用 br 分段。原提取器实测返回 null，属于正文识别缺口。来源与结构双重限定的回退加入后，实际网页提取出 16,629 字符正文，规范化和 chunk 解析均产生非空结果。真实网页仅保存在临时目录。 新增 4 项最小合成页面回归，覆盖正文/链接/强调/脚注保留、站点装饰排除、安全清洗、精确域名、必要结构反例与通用提取优先。因本机 Xcode 许可未接受，Flutter 测试在原生依赖构建前失败；仅为命令设置 Command Line Tools 目录仍未解决 hook 的 SDK 获取。改用临时独立 Dart 测试运行器，将两个纯解析测试文件的测试入口替换为 package:test ，断言不变，合计 38 项通过；5 个变更 Dart 文件静态分析无问题。此结果不代表 Flutter worker、界面或新构建真机验证已通过。 修复已接入文章详情补抓与后台全文抓取。仍需在包含修复的新构建中重新打开该空正文文章，确认实际阅读显示；不通过手工写入运行中数据库代替应用链路。 复测入口与边界： 代码位置： lib/utils/article_content_compatibility.dart 的 ext"
 },
 {
  "path": "index.html",
