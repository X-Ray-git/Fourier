...
- macOS 静默订阅源批量导出尚待用户运行确认：入口只能在汇总视图出现，进入后默认空选；卡片勾选标识不能覆盖当前文章选中态，点击卡片仍能阅读。检查全选/全不选覆盖当前完整筛选集合，四个导出动作齐全；纯复制/保存后保留选择，组合动作完整成功后退出并清空详情。保存取消或失败不能标为已读；`Cmd+Z` 应一次恢复整批且切换页面后仍有效，`Shift+Cmd+Z` 应一次重做。导出的 Markdown 不应有总标题，文章顺序正确并由 `---` 分隔。远端分块、补偿失败和部分 Undo/Redo 已由可注入的同步协调器自动化测试覆盖；若真实网络中出现部分成功提示，本地列表、菜单篇数和刷新后的远端状态应保持一致。
- GitHub 发布 Actions 已迁移到 Node 24 对应的 `checkout@v7`、`setup-java@v6`、`upload-artifact@v7` 和 `download-artifact@v8`，本地 YAML、脚本语法和完整 Flutter 测试已通过，但尚未触发真实 tag 构建。下一次用户明确允许发布时，应确认 Android、macOS arm64 和 GitHub Release 三段均成功，并且不再出现 Node 20 弃用警告。
...
- Android 垃圾拦截卡片横滑时，保留/移除背景必须完整填充卡片真实让出的圆角区域；滑动区域最外侧仍应保持卡片的 `16px` 圆角，不应残留页面底色、空带或直角边线。
