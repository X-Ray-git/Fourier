...
决策：纯 Flutter 组件和状态规则跨平台复用，包括 `AppGlassSurface`、`AppGlassCompactSwitch`、移动底部面板以及翻译/摘要语义；`NSGlassEffectView`、窗口后方取样和红黄绿等 AppKit 能力继续隔离。固定、低数量浮动控件可使用 Flutter 模糊玻璃，文章卡片等密集元素继续使用轻量 `ArticleCardChrome`。Android 主导航改成图标式悬浮胶囊，最近阅读移入订阅源二级入口；目录与订阅源设置共用 `AppMobileGlassSheet`。物理屏幕圆角的同心参数等整体几何稳定后再从目标设备反推。
