...
window.WIKI_SEARCH_MANIFEST = "17297d66c8d8dc95";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "2026-09-25 历史候选发布的工作流权限补修 Publish Release 36118746311 的静态检查、测试、Android/macOS 冒烟均成功；prepare 原子推送失败，GitHub 明确拒绝历史 v2.4.1 标签中与默认分支不同的 internal-release.yml（GITHUB_TOKEN 没有 workflows 写权限）。远端 main 未前移且没有 v2.4.1 tag，发布 worker 被跳过。这是新增发布流程遗漏的权限边界，不是应用构建失败。 历史候选 worktree 现在只同步当前 main 已批准的 .github/workflows、release.sh、prepare_release.py 和其回归脚本，再生成发布元数据；应用及依赖仍来自 92ddd94。无需扩大令牌权限，不改变原发布范围。测试加入远端 pre-receive hook 拒绝工作流树差异，包含新增/修改/删除工作流场景：修复前 4 项中历史候选用例失败，修复后全部通过，并断言应用代码仍为原候选、工作流树与 main 相同。actionlint 与 diff 检查通过。提交修复后重新 dispatch 同一 v2.4.1 候选，不在旧工作流定义上盲目 rerun。 2026-09-25 发布中断根因与完整云端流程 用户再次发现 v2.4.1 没有 Release。查证预检 36114106555 对 92ddd94 成功，最新 Release 仍 v2.4.0，既无 v2.4.1 tag 也无正式发布 run。后台日志停在推送候选；无法确认进程具体终止原因，但原链路依赖本机等待后创建 tag，预检不会自己发布。此前将 nohup 后台启动等同于可靠交接是执行错误，文档允许该方式也是缺陷。 修复：release.sh --push 只提交完整 Publish Release 请求，父工作流依次直接调用可复用预检、云端 prepare、可复用构建发布；GITHUB_TOKEN 推 tag 不触发下游的问题由直接调用解决。末尾核验非草稿/非预发布及三个非空 uploaded 资产。候选 SHA 固定，main 前移失败，main/tag 原子推送，已有 tag 核对候选/说明后复用，不可改写。补发原 92ddd94 候选，不把后续依赖升级混入安装包；main 仅同步发布元数据。 验证：actionlint 检查所有工作流通过；bash 语法检查通过；4 项隔离 Git 仓库测试覆盖正常发布、重跑幂等、历史候选隔离、main 前移/版本回退/错误说明拦截，以及本机 dispatch 到正确云端入口后不创建本地 tag。正式云端运行结果不由本地测试替代；按用户约定只确认完整流程启动，不持续等待完成。 2026-09-25 Xcode 开发组件初始化复核 用户自行执行 sudo xcodebuild -runFirstLaunch 后，xcodebuild -checkFirstLaunchStatus 从退出码 69 变为 0；CoreSimulator 从 1051.55 更新到 1171.7，CoreDevice 从 518.33 更新到 642.16。重新 flutter build macos --release --no-pub 成功，构建日志不再出现此前插件符号缺失和模拟器版本过旧的报错；只剩 Xcode 27 支持 arm64-only 的配置提示。本次没有额外更改全局 Flutter 配置，也没有要求重启或重装 Xcode。用户随后确认 flutter run 已不再出现大量红字，环境待办关闭；Dart 包新版可用提醒仍是另一类正常提示。用户授权文档与 Git 维护后提交推送，明确不打包、不发布。 2026-09-25 依赖维护第一批 用户授权按评估推进。 pub outdated 初始报告 59 个锁定旧版的可升级依赖、18 个受约束限制的依赖（含传递包）；不是 77 个独立业务库需要直接迁移。定向 upgrade 预览仅升级 Dio，显式提高缓存管理器最低版本后成功解出新版及必需 uuid。最终锁文件仅三包变化：dio 5.9.2→5.11.1、flutter_cache_manager 3.4.1→3.4.5、uuid 4.5.3→4.6.0。无 dependency_overrides，不变更 Flutter SDK。 全量 flutter test --no-pub 387 项通过（包含图片超时与缓存回归）；dart analyze lib test 无 error/warning，59 个既有 info。本地 HttpServer 冒烟验证 Dio JSON、流式响应、取消请求及取消后正常请求通过；未访问真实账号/API、未清理用户缓存。依赖解析仍提示 76 个新版可用，不将提醒数量当成故障或升级目标。macOS Release --no-pub 构建成功（35.0MB），未运行新包；未进行 Android 构建。构建同时捕捉本机 Xcode 环境诊断：DVTCoreDeviceCore 加载时 CoreDevice 符号缺失；CoreSimulator 当前 1051.55.0 低于 Xcode 所需 1171.7.0，模拟器支持被禁用。这些是设备开发组件问题，不是 Dart 包可升级提示；此时未修改系统开发工具；后续已由用户完成初始化并通过复核，见本页“Xcode 开发组件初始化复核”。 2026-09-25 v2.4.1 发布授权 用户确认采用 Patch 版本 v2.4.1（构建号 40），授权维护文档、分批提交和推送发布。范围为共享选择面板返回按钮对齐、相关文章主题横幅、审核正文理由及共享信息卡。沿用本轮已通过的 3 项控件测试、15 项文本选择回归和定向静态分析；真人视觉待验收，不将发布授权等同于验收通过。启动 release.sh 完整预检及发布链路，按用户要求不持续等待最终构建完成。 2026-09-25 主题横幅与审核正文理由 确认左侧卡片不变后，将事件主题在所有宽度下改为列表上方同宽横幅；在 ArticlePageView 现有 isReviewContext 下插入非空理由卡，位于摘要之前，不受 showSummary 控制。抽出 ArticleInfoCard，原摘要的数据订阅与显隐仍由 _SummaryCard 负责，共享组件沿用原摘要可选 HTML 渲染与纯文本规范化。MacArticleDetailStack 的相关文章页未传审核上下文，故不携带审核理由；普通阅读入口默认上下文仍为 false。 两文件定向 analyzer 无问题，既有 html_chunk_selection_test.dart 15 项通过。这是相关文本选择回归，不能代替新理由卡的人工视觉/复制验收；本次未启动应用、未修改正式数据、未提交推送或发布。视觉清单 READER-INFO-01 已追加。 2026-09-25 共享选择面板返回按钮对齐 用户指出已读/未读选择面板存在目录此前相同的左下偏移；检查发现六个入口共享 AppGlassMorp"
 },
 {
  "path": "index.html",
