...
class YouTubeSabrPlayer extends StatefulWidget {
  ...
  @override
  State<YouTubeSabrPlayer> createState() => _YouTubeSabrPlayerState();
}

class _YouTubeSabrPlayerState extends State<YouTubeSabrPlayer> {
  ...
  Future<void> _togglePlayback() async {
    await _controller?.runJavaScript(
      'globalThis.AutoFoloYouTubeControls?.togglePlayPause();',
    );
  }
  ...
  Future<void> _startPlayback() async {
    ...
    try {
      final playerUri = await YouTubePlaybackServer.playerUri(
        widget.info.videoId,
      );
      ...
      if (kDebugMode) {
        await controller.setOnConsoleMessage((message) {
          debugPrint(
            '[YouTubeSabrConsole ${message.level.name}] ${message.message}',
          );
        });
      }
  ...
  void _fallback(String reason, [StackTrace? stackTrace]) {
    ...
    if (kDebugMode) {
      debugPrint('[YouTubeSabrFallback] $reason');
