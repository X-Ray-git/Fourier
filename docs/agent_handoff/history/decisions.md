...
决策：tooltip overlay 改为读取目标和气泡真实尺寸，由共享 `SingleChildLayoutDelegate` 定位。气泡始终保留四周 `8px` 安全边距；默认位置空间不足时，下方翻到上方、右侧翻到左侧，其余水平偏移直接向窗口内 clamp。视觉材质、等待时间和业务调用位置不变。

后果：所有 `AppGlassTooltip` 自动获得一致的防截断行为。新增测试覆盖右上角长文案和右下角右侧 tooltip；不要为单个按钮重新引入硬编码位置。
