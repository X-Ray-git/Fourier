...
- 来源专属正文兼容已集中到 `ArticleContentCompatibility`。Hugging Face Blog 的 `BlogAuthorsByline` 会转换为单个紧凑作者 chunk，以 `36px` 圆形头像、姓名和账号横向换行展示；站点装饰头像堆叠仍移除。原文与已保存译文都在解析前走同一规范化路径，历史文章不需要数据库迁移。
