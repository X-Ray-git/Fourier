...
- 自动 AI 队列：刷新后快速把等待翻译/摘要的文章标为已读，后台等待数量应下降且文章不应长期停留在“翻译中”；已经进入处理批次的任务继续完成属于预期。
