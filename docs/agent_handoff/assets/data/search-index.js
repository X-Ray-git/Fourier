...
window.WIKI_SEARCH_MANIFEST = "f9f28c59f219c0b2";
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "验证记录 2026-08-18 CoderBill 邮件兼容与 Android 返回入口 CoderBill Circle 邮件兼容使用 feedId/category + 模板结构签名 双重门禁；定向测试覆盖页脚、应用推广、邮件样式清理，以及正文、主操作链接、图片尺寸和真实数据表保留。另以本机 13 份真实缓存快照做过只读结构核对，结果符合预期；快照正文和私人内容没有写入测试、文档或 Git。 正文规范化上下文现贯穿渲染、历史/新译文、摘要、过滤、图片预取、长度估算与 Markdown 导出，缓存摘要也包含 URL/feed/category。 flutter test --no-pub 共 242 项通过； dart analyze lib test 为 0 error / 0 warning，仅保留 59 条既有 prefer_initializing_formals info； git diff --check 通过。 仍需用户运行后视觉检查一篇普通 CoderBill 通知和一篇较长邮件：确认没有竖向长按钮、页脚/推广已消失，正文、链接、图片尺寸和真正的数据表仍正确。Android 需确认普通页面及文章详情使用系统返回键/边缘手势可正常返回，顶部不再出现隐式箭头；这些视觉与真机交互未由自动测试替代。 2026-08-12 macOS 能耗诊断 当前正式版在省电模式、无 LLM 请求且已运行约一天时，短时空闲采样约为 0.0%–0.1% CPU ； pmset 未发现 Fourier 持有防休眠或网络活跃断言。暂未证明存在空闲永久高负载。 新增本地轮转诊断，记录 CPU、内存、低电量/温度状态、帧耗时和各后台队列计数；不记录内容或凭据。后续需使用包含本改动的 macOS 构建正常使用，待系统再次提示高耗电后检查 energy.jsonl ，判断高峰是否与刷新图片预取、同步、AI 队列或 UI 慢帧重合。 隐藏 macOS 主页面已关闭 ticker，隐藏任务中心停止每秒刷新。定向测试、 dart analyze lib test （无新增 warning/error）及 macOS Debug 构建通过；构建中只保留 video_player_avfoundation 的既有弃用 warning。 2026-08-16 Flutter 3.47.0 工具链迁移 本地与 GitHub Actions 两个构建 job 已统一为 Flutter 3.47.0 / Dart 3.13.0 ； pubspec.yaml 最低约束和 pubspec.lock 同步更新。 锁文件仅更新 SDK 解析要求的 intl 0.20.3 、 matcher 0.12.20 、 meta 1.19.0 、 test_api 0.7.12 、 vector_math 2.4.2 ，没有扩展为全依赖升级。 dart analyze lib test 为 0 error / 0 warning，仅保留 59 条既有 prefer_initializing_formals info；Flutter 3.47 新增发现的两处认证异步 warning 已用 await 修正。 flutter test --no-pub 237 项通过；Android Debug APK、macOS Debug 和 macOS Release 均构建成功，Release 主程序经 file 确认为 arm64。macOS 构建只保留 video_player_avfoundation 2.9.6 的既有弃用 warning。 Flutter 自动把 macOS deployment target 从 10.15 提升到 12.0 ，Xcode 工程三个配置保持一致。 Android 仍可构建，但 Flutter 提示 Gradle 8.14 、AGP 8.11.1 、Kotlin 2.2.20 将在后续版本停止支持；下一次 Flutter 升级前应将三者作为独立任务联动迁移。 本机 flutter doctor -v 仅报告无法列出 iOS Simulator runtime；本工程不构建 iOS。另检测到 Android SDK 与 Homebrew 各有一份 adb ，若未来真机识别异常，应先统一 PATH 中的 adb，而不是修改项目代码。 本轮提交前未补做升级后的 macOS/Android 真机视觉与交互回归；自动测试和构建已通过，后续有设备条件时再重点复查登录、视频、滚动、macOS 窗口/玻璃效果及 Android 顶部布局，不阻塞本次工具链迁移提交。 2026-08-13 YouTube 播放器 attestation 修复与生命周期 用户真机（macOS Debug）确认： 真实播放：自定义播放器成功开始播放（当前 VOD 由 WEB_EMBEDDED → SABR → MWEB 链完成，样本最终走 MWEB 直连 DASH），不再回退官方 iframe。 播放中段请求持续 206 ，不再出现试用配额耗尽后的 403 卡死。 hover 进度条下方细线已消失，拖拽/点击跳转正常。 关页（Esc）、进入相关文章、切走文章后声音立即停止（dispose + TickerMode 双路径）。 自动化验证： flutter test --no-pub 237 项通过； dart analyze lib test 无 error/warning（仅保留既有 Liquid Glass info）；运行时 npm run typecheck && npm run build 通过；macOS 与 Android Debug 构建成功。 仍需观察： 长视频中途稳定性 ：请完整观看较长视频，确认 pot 在有效期内没有中段 403（当前真 token 的 TTL 为 43200s，理论覆盖任意 VOD，但应保留一次真实长视频观察）。 SABR UMP 空响应（Shaka 1002）由 MWEB 直连兜底，属已知独立问题；WEB_EMBEDDED 直连仍返回 \"This video is unavailable\"。两者都不阻塞播放，是否继续排查见排障案例的边界说明。 Android 未验证 ：真实 embed 页 + 注入 + 代理的整条链路只验证了 macOS。代码已仅对该 YouTube WebView 会话启用 localhost mixed content，并继续依赖 Android network security config 将外部明文请求拒绝；仍需真机确认注入时序、媒体分段、文章滚动和官方播放器回退的视觉表现。 官方 iframe 回退（ WebEmbedVideoPlayer ）在新架构下仍应偶尔抽查一次，确认错误路径没有被破坏。 2026-08-04 误分类（N）标记与撤销 已完成端到端验证： 四种动作标记写入正确：拦截页 K → k 、拦截页 M → m 、拦截页 N → n_keep 、时间线 N → n_spam ，状态组合（拒绝/已读/已审）与理由保留全部符合预期， tool/inspect_user_a"
 },
 {
  "path": "index.html",
