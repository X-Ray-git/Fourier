...
- 分屏正文使用固定 `surface` header，正文从 header 下方开始。整块 header 不使用 `BackdropFilter`，避免左侧采样到相邻时间线按钮光效；右上角交互按钮仍保留各自玻璃材质。
- 分屏正文 `Scrollbar` 使用 `MacGlassScrollbarStyle.theme`：宽 `8px`、距离右边界 `2px`，仍可拖动；正文基础左右 padding 为 `11px`，最大正文宽度设置仍独立生效。
- macOS 阅读进度条位于文章 header 下边缘；底层 `1px` 细分隔线始终显示，橙色进度在其上覆盖。Android 保留原 appbar 底部行为。
