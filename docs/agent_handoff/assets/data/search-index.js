...
window.WIKI_SEARCH_INDEX = [
 ...
 },
 {
  ...
  "text": "媒体播放 媒体播放分为两类： 普通视频 （ video_player 本地播放器）与 网页平台视频 （YouTube / Bilibili，首选仓库内可复现构建的 Shaka 网页运行时，失败时回退官方 iframe）。两类播放器共用同一套播放快捷键归属与全屏语义。 媒体播放链路 文章内媒体从点击到播放的回退链路：普通视频走本地 video_player；YouTube 与 Bilibili 走内嵌播放器运行时，首选链路失败后回退官方 iframe。 文章正文媒体 HTML chunk / iframe 严格 URL 解析 普通视频 / YouTube / Bilibili InlineVideoPlayer video_player，16:9 视口 内嵌播放器运行时 YouTube.js SABR / Bilibili 匿名 API ShakaEmbedPlayer WebView / 35s 超时 / 快捷键 / 全屏 官方 iframe 回退 youtube-nocookie / player.bilibili.com 懒加载：点击后才创建 WebView / loopback 服务 播放器快捷键归属 inline player 和全屏 player 都支持空格播放/暂停，但同一时刻只能由当前可见播放器处理全局按键。 普通 inline video 与 YouTube 自定义播放器共用 ArticleVideoPlaybackShortcut ：开始播放或点击播放器会把它设为当前播放器，裸 Space /媒体播放键在文章其他区域仍控制当前播放器；切到另一播放器时转移归属，widget dispose 时释放。YouTube 的 WKWebView 焦点内外会走同一通道并做短时间去重，不得再分别维护两套全局快捷键。 进入全屏前，inline player 如果是 activePlayer ，应暂时释放该身份；全屏页在首帧后请求焦点并接管空格。退出全屏后，inline player 再恢复 activePlayer 和焦点。 不要让 inline 与全屏页面同时响应同一次空格事件，否则可能发生连续切换两次、视觉上像“空格无效”。也不要在全屏 widget 尚未挂载完成前同步请求焦点。 普通视频 视频不自动循环：controller 初始化后显式 setLooping(false) ；播放自然结束时停在最后一帧，只有用户再次主动播放才 seek 到开头后继续。这一规则同时适用于 inline 和全屏，因为二者复用同一个 controller。 文章内普通视频视口固定为 16:9 ，不再用 feed HTML 的 width/height 改变正文布局。poster 和真实视频都在黑色视口内按自身比例 contain ，允许黑边但禁止裁切、拉伸和加载后重排正文。全屏仍按视频真实比例显示。 普通视频和 YouTube 播放前共用 MediaPlayButton ：macOS hover 使用手指光标；点击后按钮保持 64x64 ，只把图标替换为转圈。初始化状态必须立即 setState ，避免网络较慢时用户误以为没有点击成功。 AnimatedOpacity 不会自动停止命中测试。inline 与全屏视频控制栏隐藏时必须同时 IgnorePointer ，否则不可见的全屏/退出按钮仍会拦截角落点击。 inline 与全屏普通视频的 VideoProgressIndicator 均已启用 allowScrubbing: true 。 video_player 内部支持点击定位和水平拖拽：拖动开始时暂时暂停，拖动中连续 seek，结束后按拖动前状态恢复。两处进度条都用 MouseRegion(SystemMouseCursors.click) 提示可交互；YouTube 进度条由 WebView/YouTube 自己管理。 macOS 全屏视频 macOS 全屏视频不显示顶部返回、旋转按钮和顶部渐变条：返回按钮与红黄绿位置冲突，旋转对桌面没有意义，底部已有退出全屏入口。Android 仍保留移动端顶部返回和横竖屏控制，不要误删为全平台统一行为。 macOS 进入全屏视频时通过 MacOSWindowControls / window_controls 原生通道临时隐藏 AppKit 红黄绿按钮，退出页面时必须恢复。 MainFlutterWindow 保存显隐状态，窗口重新布局时不得强制把按钮提前显示出来。 全屏视频键盘： Space /媒体键播放暂停， Left 后退 5 秒， Right 前进 5 秒， Esc 退出。seek 必须 clamp 到 0..duration ；只处理首次 KeyDownEvent ，不把系统按键重复当作连续快进。 YouTube Folo/Newtype 等源会返回 youtube.com 或 youtube-nocookie.com/embed/... iframe。它不是媒体文件，不能直接交给 video_player ；严格 URL 解析得到 video ID 后，macOS/Android 首选 YouTubeSabrPlayer ，失败时回退到官方 iframe。 YouTubeEmbedInfo 只识别明确的 YouTube embed/watch/shorts/live/youtu.be URL，并统一生成隐私增强 youtube-nocookie.com 地址。 YouTubeSabrPlayer 只在用户点击后启动进程内 loopback 服务并创建 WebView。本地页面打包 YouTube.js 17.2.0、googlevideo 4.0.4、bgutils-js 3.2.0 和 Shaka Player 4.16.2；依赖锁定、构建说明和完整许可见 tool/embed_video_player_runtime/ 与 THIRD_PARTY_NOTICES.md 。不要在 Dart 中 Base64 搬运媒体数据；GoogleVideo 响应必须从 loopback 代理流式送入 WebView。 SABR 播放器的控制语义要尽量对齐 InlineVideoPlayer ：单击画面只显隐控制栏，不直接切换播放；控制栏播放时 3 秒后淡出、暂停时保持可见；进度条使用应用橙色，左侧为播放和时间，右侧只保留音量、设置和全屏。设置菜单保留字幕、清晰度、语言和倍速，不重新引入网页播放器的章节、循环、画中画或保存帧等入口。WebView 内双击全屏已关闭，避免和应用现有点击语义分叉。 VOD 的 SABR UMP 可能正常结束却没有返回媒体段、重定向或上下文更新。此时先卸载失败请求、关闭 SABR 请求改写但保留同源 GoogleVideo 代理，再由同一个 Shaka 控件加载 YouTube.js 普通自适应 DASH。SABR 与普通 DASH 都失败、直播失败、35 秒内未进入 playing，或收到主 frame 错误时，才重建为 WebEmbedVideoPlayer(startOnMount: true) 。官方回退不是临时调试代"
 },
 {
 ...
 },
 {
  ...
  "text": "设计规范 本文档是 Auto Folo Wiki 的视觉与交互设计规范，供后续所有页面美化、新增组件与视觉调整遵循。设计方向参考了开源 agent 设计技能与转场参考工程（见文末来源），并结合本项目的\"离线、单文件、agent 可读\"约束做了取舍。 设计判读（五维） 当前站点在设计上的定位： 产品形态 ：产品宣传页（首页）+ 工程手册（57 个专题页）的混合站点。 视觉语言 ：现代工具 / Builder SaaS 家族（Linear / Vercel 风格）：细线边框、单一强调色、克制渐变、无衬线中文排版。 信息密度 ：高（8/10）——手册为主，允许紧凑列表与代码块。 动效强度 ：中（4/10）——只解释状态变化与引导注意，不干扰阅读。 资产依赖 ：低（2/10）——不使用截图与位图，图形全部为内联 SVG。 品牌保真 ：中（6/10）——品牌橙为唯一强调色，其余色相不做点缀。 设计令牌 色彩 强调色（唯一）：品牌橙 #f59e0b / oklch(0.769 0.165 70.1) ，深一档 #f97316 。 语义色：链接蓝 --link 、代码棕 --code-text 、危险/保留仅用于语义场景（按钮与横幅）。 中性色阶：浅色主题 9 级（ --bg 至 --text ），深色主题对应反转；全部用 CSS 变量，hex 在前作 fallback、oklch 在后覆盖。 分区横幅色：状态蓝 / 产品橙 / 架构靛 / 功能橙红 / 平台绿 / 设计紫 / 操作黄绿 / 历史棕 / 许可绿 / 关于灰——只用于横幅图标与底色，不用于正文。 反俗套：不使用紫→粉→蓝渐变；首页 hero 光晕收敛为品牌橙 + 中性深色。 配色方向（2026-08 决策）：对标 Apple 展示页的高饱和克制风格， 不做印刷质感/压 chroma/奶油纸底 ；huashu-design 的\"印刷色质感\"协议（大面积 chroma 0.01–0.04 等）不适用于本站点，其 chroma 参考仅作为选色常识，不进入令牌。 字体与排印（huashu 排印系统） 正文：系统字体栈（PingFang SC / Microsoft YaHei / Noto Sans CJK 等）， 17px （中文建议值 17–18px），行高 1.75（可被阅读设置覆盖）。 等宽：SF Mono / Menlo / Consolas，用于代码与 kbd。 标题： clamp() 流式；按 1.2 音阶 （文档站档）对齐：17 → h3 20 → h2 25 → h1 32；正文标题对比 ≥ 2.5×。 行长：正文列 max-width: 36em 居中（中文最佳 28–32 字/行）；图形/表格可超出正文列（灯箱承担大图查看）。 text-wrap: pretty （正文）与 balance （标题）提升中文断行。 数字对齐：统计数字、阅读时长徽章等使用 font-variant-numeric: tabular-nums 。 间距与圆角 8pt 网格；正文段距 10px、章节间距 34px。 圆角分层：横幅 20 / 卡片 14-16 / 控件 999（胶囊与圆形按钮）/ 图形 14；不做全局搜索替换。 阴影 两档： --shadow-soft （悬浮反馈）与 --shadow （浮层）；浮层（面板/灯箱/命令面板）用更大的投影。 动效（令牌表，移植自 huashu-design Easing 库） 令牌 cubic-bezier 用途 --ease-expo-out (0.16, 1, 0.3, 1) 主缓动：统计数字递增、区块/场景渐显、卡片 hover、灯箱入场 --ease-overshoot (0.34, 1.56, 0.64, 1) 弹性弹出：主题图标切换、播放按钮、徽章 pop-in --ease-anticipation (0.3, -0.25, 0.6, 1) 蓄力：按钮按下、面板入场（当前未启用，预留） 时长：150–700ms（渐显类 500–600ms，微交互 150–250ms）。 触发：hover（卡片上浮、图形描边）、点击（主题切换、复制成功、mockup 播放）、滚动（场景渐显、进度条）、焦点（focus-visible 可见性）。 场景化叙事：四个大类采用\"钉住空气时间\"（sticky 150vh 容器，内容钉住约半屏滚动后释放）；渐显在钉住时触发（场景顶部到达视口顶部），标题与内容同帧出现并做行级 stagger——不要用 IO 提前触发（会产生\"先见标题、内容静态出现\"的错位）。 mockup 界面演示采用 huashu「Dashboard + Cinematic」模式：默认静态界面，点击 ▶ 播放约 8 秒流程动画后自动复位；reduced-motion 下直接显示终态。 所有动画必须受 prefers-reduced-motion 保护 ；阅读设置中\"动效关\"等价于全局 no-anim 。 View Transitions 只作渐进增强，不支持时静默降级。 动画防坑清单（huashu animation-pitfalls） 不用稀有 Unicode 字符做视觉元素（黑名单： ␣ ␀ ⌘ ⌥ ⇧ ↩ ⏎ 等）；用 CSS 构造语义盒子。 含 position: absolute 子元素的容器必须显式 position: relative 。 只动画 transform / opacity （含 filter 仅用于渐显类）；数据驱动的数量用 JS 注入。 动画中每个字符都必须在所选字体中存在；emoji 需要验证字体回退。 组件协议 横幅 ：运行时由 app.js 生成（图标 + 标题 + 首段摘要 + 专题标签 + 阅读时长徽章），分区配色，含光泽扫过与入场动画。 图形 ： div.dg 包裹的内联 SVG，复用 dg-* 类与共享箭头 marker；点击打开灯箱（ <dialog> ）。 键帽 kbd ：快捷键一律用 kbd 呈现，样式由 theme.css 统一。 调用框 ：语义警示用 details.dg-callout （\"不要回退\"），默认展开。 交互反馈 ：复制按钮成功态对勾描画；搜索/命令面板 origin-aware 展开；导航当前页高亮 + aria-current 。 反俗套检查清单 新增视觉元素前逐项自查： [ ] 没有紫→粉→蓝渐变（品牌橙是唯一渐变色相） [ ] 不用 emoji 当图标；缺图标用占位或线条 SVG [ ] 不用系统字体当展示字做\"假品牌\"；字体保持系统栈 [ ] 卡片不加彩色左色条（语义警示框除外） [ ] 不堆砌阴影/玻璃；密集重复元素保持轻量 [ ] 不虚构数据、不添加占位截图 [ ] 色相数量 ≤ 4 族（品牌橙 + 中性 + 语义色） [ ] 动效有 reduced-motion 降级 可访问性自查结论（2026-08） 按 ai-friendly-web-design 要点检查： 语义结构：header/nav/main/article/footer 齐全；对话框（命令面板、阅读设置、灯箱）均带 aria-label ；导航当前"
 },
 {
 ...
 },
 {
  ...
  "text": "开发流程 面向后续 agent 与开发者的日常开发入口。当前工具链对齐 Flutter 3.44.6 （项目最低约束 Flutter >=3.44.0 、Dart ^3.12.2 ）。 快速开始 flutter pub get flutter run -d macos # macOS flutter run -d <device-id> # Android 首次配置：应用内设置页填写 Folo Session Token （Folo Web 应用的 Cookie）与 DeepSeek API Key （翻译、摘要和垃圾拦截功能需要）。 常规检查 dart analyze lib test flutter analyze lib test flutter test flutter build macos --debug 迭代时可以使用更小范围的检查： dart analyze lib/pages/article/article_page.dart flutter test test/article_model_test.dart test/feed_model_test.dart test/html_entity_utils_test.dart 不要依赖完整 dart analyze dart analyze # 不要使用 原因：仓库的 reference/ 下有复制来的参考工程；完整 dart analyze 会把它们也扫进去，并产生无关错误。CI 使用 flutter analyze --no-fatal-infos lib test 。 macOS 本地 UI 验证 flutter run -d macos 或者： flutter build macos --debug open \"build/macos/Build/Products/Debug/Auto Folo.app\" 不要把本地 macOS release 构建作为主要 UI 验证目标。本地环境可能把 release 产物视为 ad-hoc 签名或未知证书链，从而造成启动/framework 加载问题；这不一定和当前代码有关。 如果 macOS Debug 出现“构建成功但等不到 debug connection”： flutter clean flutter pub get flutter run -d macos --no-pub 当前 Debug 配置依赖 ENABLE_DEBUG_DYLIB = NO 避免 Auto Folo.debug.dylib 被 macOS 系统策略拒载。验证时应看到 A Dart VM Service on macOS is available at: 。 动画诊断 复现 M/K 、审核横滑或主时间线双击偶发瞬移时，以 Debug 埋点运行： flutter run -d macos --no-pub \\ --dart-define=AUTO_FOLO_ANIMATION_PROBE=true \\ 2>&1 | tee /tmp/auto-folo-animation.log 按页面过滤 ReviewAnimProbe 、 TimelineAnimationProbe 、 TimelineTapProbe 、 TimelineListResetProbe 、 TimelineReadStateProbe 。埋点只输出文章 id 末 8 位、动作来源、列表数量、动画阶段和慢帧耗时，不输出标题、正文、凭据或完整文章 id。不要为了普通运行长期打开该开关。 文档维护 更新当前工作对应的专题页；长期有效的取舍写入 history/decisions.md 。 用 history/historical-map.md 定位旧上下文；历史归档不追加当前事实。 编辑 wiki 页面的方法见 站点指南 。 除非确实要保留原始时间线记录，否则不要继续向完整归档追加新工作。 相关页面 测试 ：测试命令与回归覆盖要求。 故障排查 ：常见问题速查。 Git worktree ：并行 agent 工作区规则。 发布与构建 ：版本号与发布流程。 验证记录 ：开放验证项与已确认结论。"
 },
 {
 ...
 },
 {
  ...
  "title": "术语",
  "headings": [],
  "text": "术语 Auto Folo：本应用。 Folo：本应用使用的上游服务/软件生态。 时间线：本地文章列表，包含未读/全部/已读模式和过滤条件。 垃圾拦截/审核：用于审核 AI 拒绝文章的页面。 静默订阅源：默认从普通时间线计数/列表中隐藏，除非被显式选择。 Readability：拉取后的全文内容。 翻译：按文章缓存的翻译 HTML/标题。 摘要：按文章缓存的 LLM 生成摘要。 TOC：根据标题生成的文章目录。"
 },
 {
  "path": "status/current.html",
  "title": "当前状态",
  "headings": [],
  "text": "当前状态 截至 2026-08-01： main 是当前集成分支。 Android 设计迁移已通过 merge commit bd8c1b8 合入 main ；新代理应以 main 中的移动端实现为当前事实，不再等待旧 android 分支。 本地 main 仍可能领先远端；提交/推送前必须先看 git status --short --branch 。 最近一批 worktree 功能已经用 merge commit 合入 main ，保留了分支历史。 除非用户明确要求，否则不要创建 release/tag。 AGENT_HANDOFF.md 现在只作为短入口。维护型交接知识库位于 docs/agent_handoff/ 。 旧单文件时间线已拆入 docs/agent_handoff/history/archive/ ； history/chronology.md 提供全部旧章节索引， history/timeline.md 仅保留兼容入口。当前事实不得写入历史归档。 当前产品形态： Flutter 应用，使用 GetX、Hive、Dio 和本地缓存。 产品名： Auto Folo 。 Dart package 名仍是 autofolo 。 Android application id、macOS bundle id、MethodChannel 命名空间使用 io.github.xraygit.autofolo 。 这是 X-Ray 个人使用的软件，围绕 Folo 使用场景构建，但不能暗示官方 Folo 所有权。 macOS 使用 Folo 官方网页 + loopback callback；Android 动态提供 Email、Google、GitHub 登录，其中社交方式使用 Better Auth Expo 同类的系统浏览器 + 精确 folo://autofolo-auth 回调。手动长期 Session Token 与 version 1 配置导入继续兼容。当前是单一活动账号，本地退出/Token 变化会保留普通设置并重建全部账号内容，不执行 Folo 远端 sign-out。两端设置页使用本地缓存的 Folo 头像和用户名表达登录身份，旧 Token 会按需补全资料，登录入口等待 provider 时有明确加载动画。macOS 与 Android Google 基础链路已由用户确认；Android 多方式选择、GitHub、Email/TOTP 及新账号卡视觉仍待验证。 用户最近已验证： 大批 worktree 合入后的 macOS 视觉检查可接受。 macOS 文章工具栏 hover 闪烁已修复。 从卡片右键菜单删除摘要后，文章详情能立即更新。 工具链已对齐到本地与 GitHub Actions 均使用 Flutter 3.44.6 ，项目最低约束为 Flutter >=3.44.0 、Dart ^3.12.2 。macOS 插件完成纯 SwiftPM 迁移， screen_retriever 0.2.2 修复最后一个不兼容项；CocoaPods 工程引用、Podfile/lock 和旧并行签名补丁已删除。Android Debug、macOS Debug/Release 均已本地构建通过，Release 主程序确认为 arm64。 macOS 中间时间线在具体订阅源筛选状态下，header 不再显示重复的订阅源级按钮，也不再显示清除筛选按钮；用户验证该视觉调整符合预期。 垃圾拦截页开始复用普通时间线的同步按钮和文章 AI 右键菜单：刷新按钮位置已按时间线 header 对齐，文章审核行右键也有翻译/摘要相关动作。 flutter run -d macos --no-pub 的“构建成功但等不到 debug connection”问题已定位为 Xcode Debug Dylib 被 macOS 系统策略拒载，并通过 Debug 配置 ENABLE_DEBUG_DYLIB = NO 修复。 macOS 分屏文章详情右下角圆角安全区处理已由用户验证方向正确：正文 body 使用右下角 clip 避开窗口外框圆弧，避免内容贴到应用外框圆角。 最近阅读页文章进入详情后不应显示“标为已读”。根因是 ArticleController 只读 GStorage.readStatus ，忽略了传入文章自身的 isRead ；已修为 LocalArticleDbService.readOverrideOf(entryId) ?? article.isRead 。时间线和最近阅读列表的本地已读合并也同步改为 readOverrideOf() ，同时支持本地标已读和恢复未读。 macOS 中间栏 header 的底部分隔线已统一取消，覆盖最近阅读、订阅源详情和垃圾拦截。普通时间线此前已取消同类分隔线。 ArticleCardChrome 现在统一文章卡片外壳参数：外边距、内部 padding、圆角、普通态填充和边框。普通时间线 ArticleCard 与垃圾拦截 _MacReviewRow 都应从这里取值；垃圾拦截行只保留自身动作按钮和信息块结构差异。 macOS 圆角收敛第一阶段已完成：原生窗口圆角、Flutter 外框、红黄绿圆心、侧边栏面板、大玻璃默认半径、设置/任务中心大面板和分屏文章右下角安全裁剪已联动调整。图片、代码块、小标签、 999 胶囊和 Android 端刻意未纳入本轮。 玻璃控件颜色状态第一阶段已集中到 AppGlassControlPalette ：通用玻璃按钮、文章目录项、文章摘要/翻译 pill、时间线排序/范围选项、设置页部分控件已改为通过 palette 取 hover/pressed/active/border/disabled 色。用户验证第一阶段体感基本和改动前一致，这是预期。 设置页 macOS 下拉菜单 overlay 已补局部静态底色，并让底色圆角和外框圆角对齐，解决透明背景导致选项和背后内容混在一起的问题。 macOS 固定圆形工具按钮继续由 AppGlassRoundControlChrome 统一外壳。深色按钮专属 control tint 现为普通 control alpha 的 0.52 ，有效白色 tint 约 12% ；浅色按钮采用参考工程的冷白 12% tint、 12 光学厚度、 5 blur、左上高光、 0.15 环境光、 1.2 折射/饱和度和轻微色散，并使用只绘制在形状外部的反向裁切双层阴影。tooltip、菜单、大面板和红/绿语义按钮不随这组参数改变。 macOS 主时间线与订阅源详情的 未读/全部 已放弃紧凑 switch，改为和排序相同的 34px 圆形 morph 选择按钮。两者与排序统一复用 AppGlassMorphSelectionButton ；旧 AppGlassCompactSwitch 和时间线私有排序 morph 代码已删除。展开菜单标题为“文章范围”，只包含“未读/全部”，不重新引入“已读”。 未读/全部 触发图标使用 filter_alt_rounded / filter_alt_off_rounde"
 },
 {
 ...
 },
 {
  ...
  "text": "验证记录 2026-08-04 误分类（N）标记与撤销 已完成端到端验证： 四种动作标记写入正确：拦截页 K → k 、拦截页 M → m 、拦截页 N → n_keep 、时间线 N → n_spam ，状态组合（拒绝/已读/已审）与理由保留全部符合预期， tool/inspect_user_actions.dart 核对通过。 Cmd+Z 撤销回滚验证：时间线 N 后立即撤销，文章完整还原为 拒绝=false 已读=false 已审=false act=null （forceReplace 整条替换快照生效）。 撤销栈清空行为符合预期：跨应用实例的动作（如调试构建期间的受控测试标记）不在新实例的撤销栈内，无法撤销，属预期。 误分类功能相关的旧 bug 修复后确认：已读视图/最近阅读打开文章不再出现白色覆盖（GetX 误用修复）；拦截页未点开详情直接按 M/N 也能生效（页面级处理器）。 正式版已重建（ flutter build macos --release ），134 项 Flutter 测试通过、 dart analyze lib 无 error。 用户测试产生的标记与状态污染已用一次性脚本清理，数据库恢复为 0 标记残留。 仍需留意： Android 尚未加入误分类按钮（按计划待定）；字段随共享模型落库，后续接入即用。 调试构建期间曾发生两次无日志、无崩溃报告的静默退出（无退出码捕获前），未定位根因；正式版未复现。若正式版再次出现，需在带退出码捕获的启动方式下复现。 旧版本二进制混用会静默剥掉 userAction （已接受语义），统计结果应视为\"只有真的没有假的\"。 合并审查补充修复：页面级快捷键已放行 Alt/Control/Command；撤销同步失败会重建完整操作态；所有现有文章复制路径均保留 userAction ；时间线误分类不再用立即通知绕过退场动画。新增修饰键状态和操作态快照测试； flutter test --no-pub 共 136 项通过， dart analyze lib test tool 无 error/warning，仅保留 67 条既有 info。 统计范围经用户再次确认只需近期分析：记录随 articleDb 的 5000 篇缓存和账号清理生命周期存在，不承诺永久完整历史。 仍需持续观察 2026-07-31 图片加载失败重试机制已收敛到 ArticleImageCacheService （自动指数退避 1s/2s/4s 共 3 次、失败状态落盘、全局刷新 retryFailedPrefetches 串行重扫、成功后以图片级 successRevision 精确重建）。需在 macOS/Android 真机验证：打开含失败图片的文章，确认失败占位先显示“重新加载中…”、后台自动重试成功后占位自动变成图片（无需手动点）；自动重试耗尽后占位文案变为“图片加载失败，点击重试”，手动点击仅发起一条 service 下载链并可恢复；普通位图与独立块级 SVG 都需覆盖。切到时间线做一次全局刷新，确认之前失败的图片会在缓存清理完成后重新排查，进程重启后刷新仍能重扫落盘记录。HTML 行内图片扩展和图片画廊本次未纳入重试，属预期范围边界。决策与边界见 history/decisions.md “图片加载失败的重试统一收敛到 ArticleImageCacheService”。 2026-07-31 Android 设置页 _LlmConfigCard （翻译/摘要/过滤 LLM 参数卡片）展开后第一个下拉框“模型”的浮动 label 上半部分被裁切。根因是 ExpansionTile children padding 顶部为 0 ，浮动 label 向上越出 OutlineInputBorder 约 8.6px 的部分被 ExpansionTile 内部 ClipRect 和外层 MobileSettingsPanel 的 Clip.antiAlias 切掉；已把该 padding 顶部改为 10 。需在真机分别展开翻译、摘要、过滤三个 LLM 参数卡片，确认“模型”及其他首项浮动标签完整显示，且展开动画和卡片整体布局无异常。静态分析与排查结论见 history/decisions.md “带浮动标签的设置输入控件上方必须为裁切容器预留缓冲”。 2026-07-29 YouTube SABR 首选播放器尚待用户实机验证。macOS 与 Android 分别打开真实 YouTube 文章，检查首次点击有即时 loading、能够开始播放、 播放/暂停、拖动橙色进度条、清晰度/字幕/语言/倍速切换和系统全屏可用； 单击画面应只显隐控件，播放时 3 秒淡出、暂停时保持显示。macOS 触控板在 视频画面和控制栏上纵向滑动应继续滚动文章，打开分辨率/字幕菜单后菜单 自身仍可滚动；开始播放后点击文章其他位置， Space /媒体播放键仍应控制 当前视频，同篇切换另一视频后快捷键归属应随之转移。Android 触摸纵向 滑动不得被 WebView 锁死。切换文章不能残留上一篇控制器；SABR UMP 没有 媒体段时应先尝试同控件的普通自适应 DASH，只有两者都失败才在 35 秒内 切到官方 iframe，不得打开无关浏览器页面；官方 iframe 的内部滚轮不承诺 桥接。 macOS Release 还需确认 sandbox 下 loopback server 可以绑定；Android 需确认只放行 localhost 明文而外部 HTTPS 行为不变。 macOS 静默订阅源批量导出的入口、勾选、四类动作、morph 展开和 Esc 退出已完成用户运行确认。仍需在真实网络偶发部分成功时观察：本地列表、菜单篇数、刷新后的远端状态与 Undo/Redo 历史必须只反映最终确认改变的子集；保存取消或失败不能标为已读。 GitHub 发布 Actions 已迁移到 Node 24 对应的 checkout@v7 、 setup-java@v5 、 upload-artifact@v7 和 download-artifact@v8 ，Flutter pin 也从 3.41.6 对齐为 3.44.6 。 v1.2.0 首次触发确认版本校验和 macOS arm64 全部成功，但 Android 因不存在的 setup-java@v6 在 job 初始化阶段失败，GitHub Release 因依赖关系被跳过；旧 tag 未产生 Release 后已删除，workflow 修正为实际使用 Node 24 的 @v5 ，等待重建同名 tag。重新触发后应确认 Android、macOS arm64 和 GitHub Release 三段均成功。 macOS 圆形工具按钮和文章范围 morph 已完成用户视觉检查后进入提交：深色按钮应比旧版更通透；浅色按钮应通过冷白材质、左上高光和仅位于外部的双层阴影保持可见，内部不能因阴影发灰。文章范围在主时间线与订阅源详情都应只显示“未读/全部”，深色触发图标为白色、浅色为 onSurface ，展开/收回和筛选结果应与排序共用的 morph "
 },
 {
  "path": "index.html",
  "title": "Auto Folo — 高密度信息流阅读客户端",
  "headings": [
   {
    "id": "推荐阅读顺序",
    "text": "推荐阅读顺序"
  ...
  "text": "Auto Folo — 高密度信息流阅读客户端 Auto Folo · 基于 Folo API 与 DeepSeek 驱动 让 AI 替你 筛选信息洪流 Auto Folo 把 Folo 时间线 、 长文阅读 与 可配置的 AI 工作流 放进同一个客户端：后台翻译与摘要帮助快速浏览，垃圾拦截判定进入独立审核页， 最终保留或移除仍由你决定。 开始阅读 Wiki 查看当前状态 macOS Android 个人非官方客户端 AGPL-3.0 离线工程 Wiki 非官方个人二次开发客户端，不隶属于 Folo、RSSNext 或其运营方，也不代表官方发布版本。 file://index.html · Auto Folo Wiki 核心能力 不止于阅读—— 从数据到决策的完整闭环。 从数据拉取到 AI 决策，每一层都经过精心设计。 高密度时间线 未读 / 全部 / 已读模式与长度排序，按分类、订阅源与静默订阅源筛选；macOS 原生分栏让列表与正文同步滚动。 AI 翻译与摘要 按订阅源自动翻译与生成摘要，保留 HTML 结构并支持原文 / 译文切换；模型、思考模式、temperature 与并发数独立配置，后台队列只服务仍未读的文章。 垃圾拦截审核 DeepSeek 逐篇给出保留 / 拒绝建议，被判定文章进入独立审核页：横滑、快捷键或右键菜单处理，最终决定由你做出。 长文与媒体阅读 HTML 拆块渲染、目录跳转、表格、代码块与 Markdown 复制；图片画廊与失败重试，普通视频、YouTube、Bilibili 内联播放并自动回退官方播放器。 订阅源管理 Articles / Social Media / Inbox 分组与静默订阅源，可撤销的添加 / 编辑 / 取消订阅，跨客户端同步的共享目录。 桌面体验与隐私 macOS 原生分栏、克制的 Liquid Glass、右键菜单与快捷键；凭据本地保存，配置可导出为 JSON 通过剪贴板迁移。 处理流水线 从 API 到你的屏幕， 每一步都可见。 01 拉取 从 Folo API 获取 feeds / social / inbox 三类未读文章。 02 本地合并 写入文章库并合并已读覆盖、订阅源与历史状态。 03 AI 判定 DeepSeek 给出保留 / 拒绝建议，被拒文章进入审核队列。 04 翻译与摘要 仍未读的文章按配置进入独立后台队列。 05 审核 横滑或操作菜单处理被拒文章，保留回时间线或移除。 06 同步 已读状态分批同步到 Folo，失败重试并保留待同步记录。 这本 Wiki 一个文件一页， 零依赖离线可读。 0 专题页 · 单文件即正文 0 架构与流程示意图 0 % 离线可用 · 双击即读 0 运行依赖 · 无需安装 工程 Wiki 技术手册与产品介绍， 同一份文件。 每个专题页正文以 Markdown 内嵌在页面中，编辑与阅读是同一个文件； 克隆仓库后双击本页即可离线浏览全部专题。 状态 → 当前状态 · 待办与搁置 · 验证记录 架构 → 概览 · 数据同步 · 网络 · 存储与缓存 功能 → 时间线 · 渲染 · 媒体 · 翻译 · 拦截 · 订阅 平台与设计 → macOS · Android · Liquid Glass · UI 交互 开发与操作 → 开发 · 测试 · 排障 · worktree · 发布 历史 → 决策日志 · 迁移 · 发布记录 · 主题归档 许可与致谢 → 第三方依赖 · 许可证声明 站点指南 → 如何编辑本 Wiki · 迁移映射 推荐阅读顺序 当前状态 — 最近的集成分支、产品形态与用户验证结论 待办与搁置事项 — 当前有效的未来事项索引 验证记录 — 仍需持续观察的开放验证项 开发流程 — 常用命令与检查方式 当前任务对应的专题页 需要历史证据时查 历史主题归档 ；按旧章节编号查找使用 历史时间索引 硬性规则 除非用户明确要求，否则不要创建 tag 或发布 release；发布只允许从 main 分支通过 scripts/release.sh 进行。 Flutter 项目健康检查优先使用 dart analyze lib test 、 flutter analyze lib test 和有针对性的 flutter test ；完整 dart analyze 会扫描 reference/ 并报告无关错误。 不要把密钥、API 响应、抓取的真实文章 HTML、临时脚本提交进 git。此类内容放进已忽略的 scratch/ 。 当前应用标识命名空间是 io.github.xraygit.autofolo ；历史 com.folo.* / com.autofolo 引用已经废弃。 macOS 发布产物必须保持 arm64。 维护规则 这个知识库只保留当前仍有用的知识；专题页继续变大时按子专题拆分，不要无限追加。 原始历史证据按主题保存在 history/archive/ ；提炼总结时不要删除归档。 history/timeline.html 只用于兼容旧链接，不再追加内容。 记录决策时优先写入 history/decisions.html ，包含背景、决策、后果和“不要回退”的说明。 更新文档后运行 ./scripts/docs.sh index 重新生成搜索索引，并运行 ./scripts/docs.sh check 验证链接与一致性。"
 }
];
